...
1. Prefer the live dashboard data: call **dashboard_url** for `/brain`, then use the HTTP skill on `/api/brain/cloud?range=all&source=all` and extract `json.terms[].text` or `json.denseTerms[].text` in rank order.
