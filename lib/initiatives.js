...
/** Promote an initiative to a subgoal on an existing goal (shared by API + auto-promotion). */
export async function promoteInitiativeToSubgoal(initiative, goalId, opts = {}) {
  ...
  const activityLine = opts.auto
    ...
    : `Promoted to subgoal in ${goal.id}`;
...
/**
 * Promote high-confidence open initiatives to subgoals on related active goals.
 * Runs on a 30-60 minute interval with a short age delay before eligibility.
 */
