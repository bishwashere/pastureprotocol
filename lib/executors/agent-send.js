...
export async function executeAgentSend(ctx, args = {}) {
      ...
      if (goal) {
        ...
        if (created?.subgoal) {
          delegatedTask = {
            ...
            assignee: targetAgentId,
            delegatedFrom: callerAgentId,
            status: created.subgoal.status || 'assigned',
          };
        }
      }
    } catch (persistErr) {
  ...
  try {
    ...
    if (delegatedTask) {
      delegatedTaskUpdate = recordDelegatedSubgoalReply(delegatedTask, { replySummary: reply.slice(0, 400) });
      const updatedSg = (delegatedTaskUpdate?.subgoals || []).find((sg) => String(sg.id || '') === delegatedTask.subgoalId);
    ...
    return JSON.stringify({
      ...
      delegatedTaskStatus: delegatedTaskUpdate
        ? (delegatedTaskUpdate.subgoals || []).find((sg) => String(sg.id || '') === delegatedTask.subgoalId)?.status || 'in_progress'
        : undefined,
      route: autoRoute
        ? {
            mode: 'auto',
