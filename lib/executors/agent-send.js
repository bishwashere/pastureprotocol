...
export async function executeAgentSend(ctx, args = {}) {
  ...
  if (isInternalAgent(target)) return err(`Cannot delegate to internal agent "${target}".`);
  ...
  try {
    ...
    const agentTitle = getAgentTitle(target);
    return JSON.stringify({
      agent: target,
      agentTitle: agentTitle || target,
      reply,
      skillsCalled: skillsCalled || [],
      summary: `${agentTitle || target} replied: ${reply}`,
    });
  } catch (e) {
