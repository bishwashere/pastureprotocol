 ...
 *   2. ~/.cowcode/secrets.json → secrets.github.token  (preferred for security)
...
function friendlyError(status, message) {
  ...
  if (status === 404) return `GitHub resource not found (${base}). Check the repo name, issue number, or branch.`;
  ...
  try {
    ...
    if (t && typeof t === 'string' && t.trim()) {
      if (!_warnedConfigToken) {
        ...
        console.warn('[github] Token found in config.json. For better security, move it to ~/.cowcode/secrets.json: { "github": { "token": "ghp_..." } }');
      }
...
function parseRepo(repoArg) {
  const repo = (repoArg || '').trim() || getDefaultRepo() || '';
  if (!repo.includes('/')) {
...
function normalizeListReposOwner(ownerArg) {
  ...
  if (lower === '@me' || lower === 'me' || lower === 'authenticated' || lower === 'self') return '';
  return owner;
}
...
export async function executeGithub(ctx, args, toolName) {
  ...
  if (!token) {
    return JSON.stringify({
      ...
      setup: [
        ...
        '2. Save it in ~/.cowcode/secrets.json: { "github": { "token": "ghp_..." } }',
        '   (Never commit secrets.json — it is gitignored)',
        '3. Or set GITHUB_TOKEN env var in ~/.cowcode/.env',
      ].join('\n'),
    });
  }
  ...
  try {
    switch (action) {
      ...
      case 'list_repos': {
        const owner = normalizeListReposOwner(args?.owner);
      ...
      case 'search_code': {
        ...
        if (!query) throw new Error('query required (GitHub code search syntax, e.g. "cowcode language:js")');
