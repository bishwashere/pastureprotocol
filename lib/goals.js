...
function formatDelegatedSubgoalsForPrompt(subgoals) {
  const rows = flattenSubgoals(normalizeSubgoals(subgoals || []))
    ...
    .filter((sg) => ['todo', 'doing', 'blocked'].includes(String(sg.status || 'todo').toLowerCase()));
...
export function respondToGoalUserInput(goalId, responseText) {
  ...
  appendGoalMemory(id, prev.title, [
    ...
    'Wait cleared; goal tick scheduled',
  ]);
...
export function buildGoalTickPrompt(goal, opts = {}) {
  ...
  return [
    ...
    '- Update delegated subgoal status/progress in "subgoals" when teammates finish; mark done when their reply satisfies expectedOutput.',
    '',
    ...
    '- Background curiosity passes may also add low-effort research subgoals when the mission is idle.',
    '',
