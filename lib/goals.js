...
const VALID_SUBGOAL_STATUS = new Set([
  'open',
  'assigned',
  'in_progress',
  'waiting_user',
...
const MAX_CREATED_SUBGOALS_PER_TICK = 3;
...
function normalizePlanSteps(steps) {
  ...
  return steps
    .map((step, i) => {
      ...
      const status = VALID_SUBGOAL_STATUS.has(statusRaw) ? statusRaw : 'todo';
...
function normalizeCreatedSubgoal(row, idx, opts = {}) {
  if (!row || typeof row !== 'object') return null;
  const title = summarize(row.title || row.name || '', 180);
  if (!title) return null;
  const description = summarize(row.description || row.objective || '', 400);
  ...
  const dependsOn = safeArray(row.depends_on || row.dependsOn || [], 20);
  let id = String(row.id || `sg-new-${nowMs().toString(36)}-${idx + 1}`).trim() || `sg-new-${idx + 1}`;
  const out = {
    id,
    title,
    status: 'todo',
...
/** Insert brand-new subgoals from a tick response without restructuring the existing tree. */
export function createSubgoalsFromTick(existingSubgoals, createdRaw, opts = {}) {
  const maxNew = Math.max(1, Math.min(
    MAX_CREATED_SUBGOALS_PER_TICK,
    Number(opts.maxNew) || MAX_CREATED_SUBGOALS_PER_TICK,
  ));
  const existingNorm = normalizeSubgoals(existingSubgoals || []);
  const existingFlat = flattenSubgoals(existingNorm);
  ...
  if (!Array.isArray(createdRaw) || !createdRaw.length) {
    return { subgoals: existingNorm, created };
  }

  for (let i = 0; i < createdRaw.length && created.length < maxNew; i++) {
    const normalized = normalizeCreatedSubgoal(createdRaw[i], i, opts);
  ...
  if (!created.length) return { subgoals: existingNorm, created };
  return { subgoals: [...existingNorm, ...created], created };
}
...
function formatDelegatedSubgoalsForPrompt(subgoals) {
  ...
  const rows = flattenSubgoals(normalizeSubgoals(subgoals || []))
...
function normalizeWaitCondition(wait, opts = {}) {
  ...
  if (kind === 'manual' || kind === 'partial') {
    ...
    const blockedSubgoalIds = safeArray(wait.blockedSubgoalIds || wait.appliesToSubgoalIds || [], 20);
    return {
      kind: 'partial',
      scope: waitAppliesTo,
      waitAppliesTo,
...
function getWaitBlockedSubgoalIds(waitCondition) {
  if (!isPartialWaitCondition(waitCondition)) return [];
  return safeArray(waitCondition.blockedSubgoalIds || waitCondition.appliesToSubgoalIds || [], 20);
}

function classifySubgoalScope(subgoal) {
  const hay = `${subgoal?.id || ''} ${subgoal?.title || ''} ${subgoal?.description || ''}`.toLowerCase();
...
function scopeBlockedByWaitAppliesTo(subgoalScope, waitAppliesTo) {
  if (waitAppliesTo === 'all') return true;
  if (waitAppliesTo === subgoalScope) return true;
  if (waitAppliesTo === 'implementation' && (subgoalScope === 'implementation' || subgoalScope === 'instrumentation' || subgoalScope === 'deployment')) {
    return true;
  }
  return false;
}

export function subgoalBlockedByWait(subgoal, waitCondition) {
  if (!subgoal || !isPartialWaitCondition(waitCondition)) return false;
  const blockedIds = getWaitBlockedSubgoalIds(waitCondition);
  if (blockedIds.includes(String(subgoal.id || '').trim())) return true;
  const waitAppliesTo = normalizeWaitAppliesTo(waitCondition.waitAppliesTo || waitCondition.scope, 'implementation');
  return scopeBlockedByWaitAppliesTo(classifySubgoalScope(subgoal), waitAppliesTo);
}

/** Split subgoals into wait-blocked vs still actionable during a partial/manual wait. */
export function partitionSubgoalsByWait(subgoals, waitCondition) {
  const flat = flattenSubgoals(normalizeSubgoals(subgoals || []));
  const blocked = [];
  const actionable = [];
  flat.forEach((sg) => {
    if (subgoalBlockedByWait(sg, waitCondition)) blocked.push(sg);
...
function applyWaitBlocksToSubgoals(subgoals, waitCondition) {
  if (!isPartialWaitCondition(waitCondition)) return normalizeSubgoals(subgoals || []);
  function walk(list) {
    return (list || []).map((sg) => {
      const next = { ...sg };
      if (subgoalBlockedByWait(sg, waitCondition) && String(next.status || '').toLowerCase() !== 'done') {
        next.status = 'blocked';
      }
      if (Array.isArray(next.subgoals) && next.subgoals.length) {
        next.subgoals = walk(next.subgoals);
      }
      return next;
    });
  }
  return walk(normalizeSubgoals(subgoals || []));
}
...
function formatWaitScopeForPrompt(waitCondition, subgoals) {
  ...
  return [
    ...
    actionableLines.length
      ...
      : 'Actionable branches: research, planning, createdSubgoals, initiatives (parallel safe work).',
...
export function isGoalTickPausedByWait(goal, now = nowMs()) {
  const waitEval = evaluateWaitCondition(goal?.waitCondition, { now });
...
function normalizeGoal(goal = {}) {
  ...
  return {
    ...
    lastCuriosityAt: Number(goal.lastCuriosityAt) || 0,
...
function unblockSubgoalsAfterUserResponse(subgoals) {
  return normalizeSubgoals(subgoals || []).map((sg) => {
    const status = String(sg.status || 'todo').toLowerCase();
    const next = {
      ...sg,
      status: status === 'blocked' ? 'todo' : sg.status,
      subgoals: unblockSubgoalsAfterUserResponse(sg.subgoals || []),
    };
    return next;
  });
}

export function respondToGoalUserInput(goalId, responseText) {
  const id = String(goalId || '').trim();
  const text = summarize(String(responseText || '').trim(), 500);
  if (!id) throw new Error('goal id is required');
  if (!text) throw new Error('response is required');
  const prev = getGoal(id);
  if (!prev) throw new Error(`Goal not found: ${id}`);
  const now = nowMs();
  appendGoalMemory(id, prev.title, [
    `User input received: ${text}`,
    'Wait cleared; blocked subgoals reopened; goal tick scheduled',
  ]);
  return updateGoal(id, {
    needsUserInput: '',
    waitCondition: null,
    blockedReason: '',
    subgoals: unblockSubgoalsAfterUserResponse(prev.subgoals),
    nextRunAt: now,
...
export function appendGoalMemory(goalId, title, lines) {
...
export function buildGoalTickPrompt(goal, opts = {}) {
  ...
  const delegatedSubgoalsText = formatDelegatedSubgoalsForPrompt(g.subgoals || []);
  const waitConditionText = formatWaitConditionForPrompt(g.waitCondition);
  const waitScopeText = formatWaitScopeForPrompt(g.waitCondition, g.subgoals);
  return [
    ...
    delegatedSubgoalsText ? `Open delegated assignments (track status/progress in subgoals):\n${delegatedSubgoalsText}` : '',
    waitConditionText || '',
    waitScopeText || '',
    '',
    ...
    '- agent-send creates persistent delegated subgoals on this goal (source=delegation) with assignee, due date, and expected output.',
    '- Update delegated subgoal status/progress in "subgoals" when teammates reply; use review_ready when expectedOutput appears satisfied, and done only after acceptance/verification.',
    '',
    ...
    '- Preserve existing subgoal IDs and tree shape from "Current subgoals tree". Update status/progress only in "subgoals" — do not replace a flat task list with a new nested tree.',
    '- Put genuinely new tasks in "createdSubgoals" instead of reshaping "subgoals".',
    ...
    '- Optionally set wait.blockedSubgoalIds to explicit subgoal ids blocked by the wait.',
    '- partial wait does NOT pause goal ticks; pair with needsUserInput for dashboard prompts.',
    '- While waiting, continue parallel safe work on non-blocked subgoals, createdSubgoals, and initiatives.',
    '- time and team_activity pause the entire mission until the condition is met.',
    ...
    '- Initiatives are proposals only — they land in proposed status for lead review; they do not become subgoals until approved.',
    '',
    '9) Curiosity & Next Steps',
    '- After current work, create 1-3 specific new subgoals if needed. Be proactive.',
    '- Use createdSubgoals for new work discovered this tick (research, validation, prep, follow-ups).',
    '- Each createdSubgoal needs: title, description, assignee (agent id), priority (1=highest, 5=lowest), dueInHours.',
    '- Max 3 createdSubgoals per tick. Skip duplicates of existing subgoal titles.',
    '- Background curiosity checks may log idle suggestions when a mission is quiet; they do not create subgoals or change progress.',
    '',
    ...
    '  "needsUserInput": "",',
    '  "wait": {"kind":"none|time|team_activity|partial|manual", "waitAppliesTo":"implementation|research|deployment|instrumentation|all", "blockedSubgoalIds":[], "reason":"", "untilTs":0, "eventType":"", "messageIncludes":"", "agentId":"", "targetAgentId":"", "condition":""},',
    ...
    '      "id": "research-phase",',
    '      "title": "Research",',
    '      "status": "open|assigned|in_progress|waiting_user|waiting_dependency|blocked|review_ready|done|rejected|todo|doing",',
    '      "progress": 0,',
    ...
    '  "createdSubgoals": [',
    '    {',
    '      "title": "Validate competitor signup flows",',
    '      "description": "Capture 3 examples and note friction points",',
    '      "assignee": "marketer",',
    '      "priority": 2,',
    '      "dueInHours": 24,',
    '      "depends_on": []',
...
export async function runGoalTick(goalId, opts = {}) {
  ...
  const promptBuilder = typeof opts.promptBuilder === 'function' ? opts.promptBuilder : buildGoalTickPrompt;
  const prompt = promptBuilder(started, { memoryPath, goalMemory, mode: opts.mode || 'goal_tick' });
  ...
  try {
    ...
    const patchedSubgoals = Array.isArray(parsed.subgoals)
      ? mergeSubgoalTrees(started.subgoals || [], parsed.subgoals)
      : normalizeSubgoals(started.subgoals || []);
    const waitScopedSubgoals = applyWaitBlocksToSubgoals(patchedSubgoals, nextWaitCondition);
    const createResult = createSubgoalsFromTick(waitScopedSubgoals, parsed.createdSubgoals, {
      defaultAssignee: started.ownerAgentId,
      maxNew: Math.max(1, Math.min(MAX_CREATED_SUBGOALS_PER_TICK, Number(opts.maxCreatedSubgoals) || MAX_CREATED_SUBGOALS_PER_TICK)),
    });
    const nextSubgoals = applyWaitBlocksToSubgoals(createResult.subgoals, nextWaitCondition);
    const createdSubgoals = createResult.created.map((sg) => ({
      id: sg.id,
      title: sg.title,
    ...
    const memoryLines = [
      ...
      updated.subgoals && updated.subgoals.length
        ...
        : '',
      createdSubgoals.length
        ? `New subgoals: ${createdSubgoals.map((sg) => sg.title).join(', ')}`
        : '',
    ...
    if (initiativeResult.created.length || initiativeResult.merged.length) {
      const activity = [
        initiativeResult.created.length ? `proposals created=${initiativeResult.created.length}` : '',
