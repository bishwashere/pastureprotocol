...
export function buildTurnDoneInboxDetails({ textToSend, skillsCalled, ctx }) {
  const pair = parseInternalPairJid(ctx?.jid);
  const body = String(textToSend || '').replace(/^\[CowCode\]\s*/i, '').trim().slice(0, 2000);
...
export function buildDelegationStartInboxDetails({ message, callerAgentId, targetAgentId }) {
