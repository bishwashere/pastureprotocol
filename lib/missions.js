...
import { createSuggestedTasks } from './ai-suggested-tasks.js';
import { formatUserFacingReply } from './user-facing-reply.js';
import { TASK_LABELS, addTaskLabel, normalizeTaskLabels } from './tasks.js';
  ...
  function patchList(list) {
    return (list || []).map((sg) => {
      ...
      const next = match
        ? {
          ...sg,
          status: normalizeTaskStatus(match.status || sg.status),
...
export function taskBlockedByWait(task, waitCondition) {
  ...
  if (blockedIds.includes(String(task.id || '').trim())) return true;
...
function applyWaitBlocksToTasks(tasks, waitCondition) {
  ...
  function walk(list) {
    return (list || []).map((sg) => {
      const next = { ...sg };
      if (taskBlockedByWait(sg, waitCondition) && String(next.status || '').toLowerCase() !== 'done') {
        Object.assign(next, addTaskLabel({ ...next, status: 'blocked' }, TASK_LABELS.BLOCKER));
      }
...
function writeStore(store) {
  ...
  const payload = {
    missions: Array.isArray(store.missions) ? store.missions.map(normalizeMission) : [],
    updatedAt: nowMs(),
  };
...
export function buildMissionTickPrompt(mission, opts = {}) {
  ...
  return [
    ...
    `Per-mission memory file path: ${memoryPath}`,
    missionMemory ? `Per-mission memory contents (review this first):\n${missionMemory}` : 'Per-mission memory contents: (empty)',
    ...
    '- Review the per-mission memory file contents before taking action.',
    '',
    ...
    '- Update delegated task status/progress in "tasks" when teammates reply; use review_ready when expectedOutput appears satisfied, and done only after acceptance/verification.',
    '',
    ...
    '- Each createdTask needs: title, description, assignee (agent id), priority (1=highest, 5=lowest), dueInHours.',
