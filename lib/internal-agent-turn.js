...
import { resolveSharedTurnHistory, buildPairHistoryContextBlock } from './conversation-context.js';
import { buildProjectsContextBlock, enrichMessageWithProjectContext } from './projects-context.js';
import { buildMissionsContextBlock, getMissionsDiscoveryIntentHint } from './missions-context.js';
import { buildDelegatedTasksContextBlock } from './delegated-tasks.js';
import { buildProjectWorkflowContextBlock } from './project-workflow.js';
import { getGithubSourceIntentHint } from './github-context.js';
import { appendUserFacingPrompt, buildToolAuditRewriteInstruction } from './user-reply-style.js';
import { formatUserFacingReply, looksLikeToolAuditReply } from './user-facing-reply.js';
...
import { appendExchange, readLastPrivateExchanges, DEFAULT_CHAT_HISTORY_EXCHANGES } from './chat-log.js';
...
export async function runInternalAgentTurn({
  ...
  let textToSend = (turn?.textToSend || '').trim();
  let skillsCalled = Array.isArray(turn?.skillsCalled) ? turn.skillsCalled : [];
  if (
    ...
    try {
      ...
      if (candidate && !looksLikeToolAuditReply(candidate)) {
        textToSend = (rewrite?.textToSend || '').trim();
