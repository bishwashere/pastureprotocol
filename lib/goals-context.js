  ...
  try {
    ...
    if (workRequest) {
      lines.push(
        ...
        '- Treat this as continuing the goal above — use tools (browse, github, memory, read, search) as needed for the **next plan step or subgoal**, not idle clarification.',
        '- Use the related project URL and your skills to gather facts; only ask the user to confirm after you report what you found.',
        '- You may run the goal on the next autonomous tick; for chat, answer from investigation now.',
      );
    }
    return lines.join('\n');
  } catch (err) {
...
export function getGoalsDiscoveryIntentHint(userText, historyMessages, enabledSkillIds, agentId = 'main') {
  ...
  return {
    mode: 'tool',
    skills,
    plan: `Continue goal "${title}": use tools to investigate before asking the user to pick repo/path/source.`,
    answer_style: 'normal',
  };
}
