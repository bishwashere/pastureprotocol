  ...
  try {
    ...
    if (workRequest) {
      lines.push(
        ...
        '- Treat this as continuing the goal above — use tools for the next plan step or subgoal, not idle clarification.',
        '- Use the related project URL when relevant; ask the user to confirm only after you report what you found.',
        '- You may run the goal on the next autonomous tick; for chat, answer from what you learn now.',
      );
    }
    return lines.join('\n');
  } catch (err) {
...
export function getGoalsDiscoveryIntentHint(userText, historyMessages, enabledSkillIds, agentId = 'main') {
  ...
  return {
    mode: 'tool',
    skills,
    plan: `Continue goal "${title}": use tools to advance the goal before asking the user to pick repo/path/source.`,
    answer_style: 'normal',
  };
}
