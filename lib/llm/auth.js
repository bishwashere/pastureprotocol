...
const OPENAI_CODEX_AUTH_PATH = join(homedir(), '.codex', 'auth.json');
...
export function isCodexManagedChatgptAuth(auth = {}, entry = {}) {
  const type = String(auth?.type || '').trim().toLowerCase();
  const provider = entry?.provider || auth?.provider;
  return type === 'chatgpt' && isOpenAiProvider(provider);
}

function withProviderAuthDefaults(auth = {}, entry = {}) {
  const next = { ...auth };
  const provider = String(entry.provider || next.provider || '').trim().toLowerCase();
  if (isOpenAiProvider(provider) && (next.type === 'chatgpt' || next.type === 'codex_chatgpt')) {
    return { type: 'chatgpt', provider: 'openai' };
  }
  if (isOpenAiProvider(provider) && next.type === 'oauth') {
    next.provider = 'openai';
    next.cache = next.cache || next.account || 'openai';
  }
...
function readOpenAiCodexAuthToken() {
  try {
    const data = JSON.parse(readFileSync(OPENAI_CODEX_AUTH_PATH, 'utf8'));
    ...
    return {
      provider: 'openai',
      account: tokens.account_id || null,
      token_type: 'Bearer',
      access_token: access,
...
export function importOpenAiBrowserAuth(auth = {}) {
  const token = readOpenAiCodexAuthToken();
  if (!token) {
    throw new Error('OpenAI browser login is not available yet on this machine. Sign in with Codex/ChatGPT once, then click Login once again.');
  }
  const cache = auth.cache || auth.account || 'openai';
  const path = writeLlmAuthToken(cache, { ...token, account: auth.account || token.account });
...
export function hasUsableLlmAuth(auth = {}, entry = {}) {
  ...
  if (type === 'chatgpt') return isCodexManagedChatgptAuth(auth, entry);
...
export async function resolveLlmAuthHeaders(auth = {}, entry = {}) {
  ...
  // ChatGPT subscription credentials are owned by Codex App Server and must
  // not be sent through the generic OpenAI-compatible HTTP path.
