...
function withProviderAuthDefaults(auth = {}, entry = {}) {
  ...
  if (isOpenAiProvider(provider) && (next.type === 'chatgpt' || next.type === 'codex_chatgpt')) {
    // ChatGPT subscription auth is owned by Codex App Server. Keep it
    // deliberately separate from generic OAuth fields and token caches so a
    // ChatGPT token can never be sent to the Platform API by accident.
...
export function hasUsableLlmAuth(auth = {}, entry = {}) {
  ...
  // Codex resolves the live ChatGPT account asynchronously when the model is
  // called. Retain the model here; the dedicated transport reports a clear
  // auth error (and permits normal model fallback) when signed out.
...
export async function resolveLlmAuthHeaders(auth = {}, entry = {}) {
  ...
  // ChatGPT subscription credentials are never exposed to this generic HTTP
  // path. Codex App Server owns their storage, refresh, and request transport.
