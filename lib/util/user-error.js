...
function formatLlmFailuresForUser(failures) {
  ...
  if (!parts.length) {
    const first = list[0];
    const detail = String(first.message || 'unknown error').split('\n')[0].replace(/[.\s]+$/, '');
    parts.push(`${modelLabel(first)} failed: ${detail}`);
  }
  return `I couldn't answer because ${parts.join(', and ')}.`;
}

function withTaskWorklogRecovery(err, message) {
  const count = Number(err?.taskWorklogCheckpointCount);
  if (!Number.isFinite(count) || count < 0) return message;
  const base = String(message || '').trim();
  if (count === 0) return `${base} No durable checkpoint was saved before it stopped.`;
  const saved = Math.floor(count);
  return `${base} I saved ${saved} checkpoint${saved === 1 ? '' : 's'}, so ask me to continue and I can resume from them.`;
}
...
export function toUserMessage(err) {
  ...
  if (llmFailureMessage) return withTaskWorklogRecovery(err, llmFailureMessage);
  ...
  if (/timeout/i.test(msg)) return withTaskWorklogRecovery(err, "That took too long. Please try again.");
