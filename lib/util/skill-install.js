...
import { beginCliSession, endCliSession, statusOk } from './cli-banner.js';
...
/** Hidden from pasture skills wizard (implicit / team-managed). */
const WIZARD_HIDDEN_SKILLS = new Set([
  'agent-send',
  'background-tasks',
...
/**
 * Display label for a skill in the CLI wizard.
 * @param {string} installDir
 * @param {string} skillId
 */
export function getSkillDisplayLabel(installDir, skillId) {
  const meta = SKILL_INSTALL_PROMPTS[skillId];
  if (meta?.prompt) return meta.prompt;
  if (skillId === 'github') return 'GitHub';
  if (skillId === 'gog') return 'Google Workspace';
  if (skillId === 'gmail') return 'Gmail';
  if (skillId === 'calendar') return 'Calendar';
  for (const name of SKILL_MD_NAMES) {
    const mdPath = join(installDir, 'skills', skillId, name);
...
function readEnabledSkillIds() {
  ensureStateDir();
  const configPath = getConfigPath();
  try {
    if (existsSync(configPath)) {
      const config = JSON.parse(readFileSync(configPath, 'utf8'));
      return Array.isArray(config.skills?.enabled) ? config.skills.enabled : [];
    }
  } catch (_) {}
  return [];
}

function isSkillFullyEnabled(rawSkillId, enabled) {
  const canonical = normalizeSkillId(rawSkillId);
  const enableList = getSkillsToEnable(rawSkillId, canonical);
  return enableList.every((id) => enabled.includes(id));
}

/**
 * Skill ids shown in pasture skills (wizard / list).
 * @param {string} installDir
 * @returns {string[]}
 */
export function listSkillCatalog(installDir) {
  const skillsDir = join(installDir, 'skills');
  if (!existsSync(skillsDir)) return [];
  return readdirSync(skillsDir, { withFileTypes: true })
    .filter((d) => d.isDirectory())
    .map((d) => d.name)
    .filter((id) => skillExists(installDir, id))
    .filter((id) => !WIZARD_HIDDEN_SKILLS.has(id))
    .sort((a, b) => a.localeCompare(b));
}

/**
 * Print enabled / available skills for pasture skills list.
 * @param {string} installDir
 */
export function printSkillList(installDir) {
  const catalog = listSkillCatalog(installDir);
  const enabled = readEnabledSkillIds();
  console.log('');
  console.log('Pasture skills');
  console.log('──────────────');
  if (catalog.length === 0) {
    console.log('  No skills found in this install.');
    console.log('');
    return;
  }
  for (const id of catalog) {
    const label = getSkillDisplayLabel(installDir, id);
    const status = isSkillFullyEnabled(id, enabled) ? 'enabled' : 'not enabled';
    console.log(`  ${id.padEnd(16)} ${label} (${status})`);
  }
  console.log('');
  console.log('  Add:    pasture skills install <skill-id>   (or run pasture skills)');
  console.log('  Remove: pasture skills remove <skill-id>');
  console.log('');
}

function getRemovableSkillIds(installDir, enabled) {
  return listSkillCatalog(installDir).filter((id) => {
    if (PROTECTED_SKILLS.has(id)) return false;
    const removeList = getSkillsToRemove(id, normalizeSkillId(id));
    return removeList.some((rid) => enabled.includes(rid));
  });
}

async function pickFromNumberedList(items, askFn, prompt) {
  if (items.length === 0) return null;
  items.forEach((item, i) => {
    console.log(`  ${i + 1}. ${item.line}`);
  });
  console.log('');
  const pick = await askFn(prompt);
...
async function wizardAddSkill(installDir, deps) {
  const askFn = deps.ask || ask;
  const enabled = readEnabledSkillIds();
  const catalog = listSkillCatalog(installDir);
  const candidates = catalog.map((id) => ({
    value: id,
    line: `${id} (${getSkillDisplayLabel(installDir, id)}) — ${isSkillFullyEnabled(id, enabled) ? 'enabled' : 'not enabled'}`,
  }));

  console.log('');
  console.log('Add a skill');
  console.log('───────────');
  const skillId = await pickFromNumberedList(
    candidates,
    askFn,
    'Pick a number or enter skill id (e.g. github, google): ',
  );
  if (!skillId) {
  ...
  const result = await runSkillInstall(skillId, installDir, deps);
  if (!result.ok) {
    console.error('pasture:', result.message);
    return { ok: false, changed: false };
  }
  if (deps.onSkillChanged) await deps.onSkillChanged();
  return { ok: true, changed: true };
}

async function wizardRemoveSkill(installDir, deps) {
  const askFn = deps.ask || ask;
  const enabled = readEnabledSkillIds();
  const removable = getRemovableSkillIds(installDir, enabled);
  const candidates = removable.map((id) => ({
    value: id,
    line: `${id} (${getSkillDisplayLabel(installDir, id)})`,
  }));
  ...
  if (candidates.length === 0) {
    console.log('  No removable skills are enabled.');
    return { ok: true, changed: false };
  }
  ...
  const skillId = await pickFromNumberedList(
    candidates,
    askFn,
    'Pick a number or enter skill id: ',
  );
  if (!skillId) {
    console.log('  Cancelled.');
    return { ok: true, changed: false };
  ...
  const result = await runSkillRemove(skillId, installDir, deps);
  if (!result.ok) {
    console.error('pasture:', result.message);
    return { ok: false, changed: false };
  }
...
/**
 * Interactive pasture skills wizard (pasture skills).
 * @param {string} installDir
 * @param {{ ask?: Function, promptSecret?: Function, onSkillChanged?: Function }} [deps]
 * @returns {Promise<{ ok: boolean, changed: boolean }>}
 */
export async function runSkillsWizard(installDir, deps = {}) {
  const askFn = deps.ask || ask;

  beginCliSession(deps);
  let changed = false;

  while (true) {
    console.log('Pasture skills — add, remove, or list agent skills.');
    console.log('Direct: pasture skills install <id> | pasture skills remove <id>');
    console.log('');
    console.log('What would you like to do?');
    console.log('  1. Add a skill');
    console.log('  2. Remove a skill');
    console.log('  3. List skills');
    console.log('  q. Quit');
    console.log('');
    const action = await askFn('Choose [1]: ');

    if (!action || action === '1') {
      const result = await wizardAddSkill(installDir, deps);
      if (!result.ok) continue;
      if (result.changed) changed = true;
      continue;
    }
    if (action === '2') {
      const result = await wizardRemoveSkill(installDir, deps);
      if (!result.ok) continue;
      if (result.changed) changed = true;
      continue;
    }
    if (action === '3') {
      printSkillList(installDir);
      continue;
    }
    if (/^q$/i.test(action.trim())) {
      break;
    }
    console.log('  Unknown option. Enter 1, 2, 3, or q.');
  }
...
async function setupGithub(config, deps) {
  ...
  console.log(statusOk('  ✓ GitHub token saved.'));
...
async function setupGoogle(config, deps) {
  ...
  console.log(statusOk(`  ✓ Google account set: ${account}`));
  return { ok: true };
}
...
export async function runSkillInstall(rawSkillId, installDir, deps = {}) {
  ...
  if (meta?.envVars?.length) {
    ...
    console.log(statusOk('  ✓ Saved credentials.'));
  }
  ...
  for (const id of enableList) {
    if (!nextEnabled.includes(id)) {
      nextEnabled.push(id);
      console.log(statusOk(`  ✓ Enabled ${id}.`));
    }
  }
  ...
  console.log(statusOk('  ✓ Config updated.'));
...
export async function runSkillRemove(rawSkillId, installDir, deps = {}) {
  ...
  for (const id of toRemove) {
    console.log(statusOk(`  ✓ Disabled ${id}.`));
  }
  ...
  if (clearCredentials) {
    ...
    writeFileSync(envPath, stringifyEnv(env), 'utf8');
    writeSecrets(secrets);
    console.log(statusOk('  ✓ Removed saved credentials.'));
  }
