...
const WRAPPER_TOKENS = new Set([
  ...
  'node', 'node.exe', 'python', 'python3', 'python.exe', 'powershell', 'powershell.exe', 'pwsh', 'pwsh.exe',
  '/bin/sh', '/bin/bash', '/bin/zsh', '/usr/bin/env', 'env',
]);

const SCRIPT_EXT = /\.(sh|bash|py|pl|rb|js|mjs|cjs|ts|php|ps1|cmd|bat|command)$/i;
...
export function extractScriptPath(command) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (isAbsoluteAnyPath(t) || t.startsWith('./') || t.startsWith('../') || SCRIPT_EXT.test(t)) {
      return t;
    }
    break;
  }
...
export function describeScriptContent(content, filePath = '') {
  ...
  if (!text && filePath) {
    text = humanizeScriptName(basenameAnyPath(filePath));
  }
...
export function inferCommandSummary(command, scriptPath = null) {
  ...
  let args = tokens;
  if (path) {
    const base = basenameAnyPath(path);
    const idx = tokens.findIndex((t) => t === path || t.endsWith(path) || basenameAnyPath(t) === base);
    if (idx >= 0) args = tokens.slice(idx + 1);
  }
  const argText = args.filter((a) => a && !a.startsWith('(')).join(' ').trim();
  const name = path ? basenameAnyPath(path) : '';
  const parts = [];
...
export function describeScriptFile(filePath) {
  ...
  try {
    ...
    return {
      ok: true,
      scriptPath: resolved,
      description: description || humanizeScriptName(basenameAnyPath(resolved)),
      purpose: purpose || description || humanizeScriptName(basenameAnyPath(resolved)),
    };
  } catch (err) {
    return {
      ok: false,
