...
export function enrichCrontabEntries(entries) {
  ...
  return entries.map((entry) => {
    ...
    const description = mergeDescriptions(
      info.ok ? info.description : null,
      entry.crontabNote,
      command,
      scriptPath,
    ) || (info.error && entry.crontabNote ? entry.crontabNote : info.error || null);

    return {
      ...entry,
      scriptPath,
      description,
      descriptionError: description ? null : (info.error || null),
    };
  });
}
