 ...
 * Writes JSONL to ~/.pasture/daily-logs/timing/YYYY-MM-DD.jsonl and mirrors summary lines to runtime logs as [timing].
 */

import { appendFileSync, existsSync, mkdirSync, symlinkSync, unlinkSync } from 'fs';
...
function appendTimingRow(row) {
  try {
    ...
    refreshCurrentTimingLink(dir, basename(logPath));
...
function refreshCurrentTimingLink(dir, targetName) {
  try {
...
// Event types that are surfaced in runtime logs. All events always go to JSONL.
const CONSOLE_EVENT_TYPES = new Set(['request_start', 'request_end']);

const FLOW_STEP_BY_PHASE = Object.freeze({
  fast_turn_triage: '2.5 FAST TRIAGE',
