...
import { statusOk } from './cli-banner.js';
...
export function runUninstall(opts = {}) {
  const installDir = opts.installDir || join(homedir(), '.local', 'share', 'pastureprotocol');
  const binDir = join(homedir(), '.local', 'bin');
  const stateDir = join(homedir(), '.pasture');
  const removeState = opts.removeState !== false;

  console.log('  Pasture Protocol Uninstaller');
  console.log('  -------------------');
  console.log('');

  console.log('  ► Stopping pm2 process...');
  stopPm2ForUninstall();
  console.log(statusOk('  ✓ Done.'));

  for (const name of ['pasture.cmd', 'pasture.cmd', 'pasture']) {
  ...
  console.log('');
  console.log('  ------------------------------------------------');
  console.log(statusOk('  ✓ Pasture Protocol has been successfully uninstalled.'));
  console.log('');
}
