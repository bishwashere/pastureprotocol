...
/**
 * Resolve state directory. Override with PASTURE_STATE_DIR env var.
...
export function getStateDir() {
  const override = process.env.PASTURE_STATE_DIR?.trim();
  if (override) return isAbsolute(override) ? override : join(homedir(), override);
...
/** Secure per-run checkpoint files for long-running agent work. */
export function getTaskWorklogsDir() {
  return join(getStateDir(), 'task-worklogs');
}

/**
 * Path for a validated worklog id. Callers that accept external input must
 * validate it before using this low-level path helper.
 */
export function getTaskWorklogPath(taskWorklogId) {
  const id = String(taskWorklogId || '').trim();
  if (!/^[0-9a-zA-Z][0-9a-zA-Z_-]{0,127}$/.test(id)) {
    throw new TypeError('Invalid task worklog id.');
  }
  return join(getTaskWorklogsDir(), `${id}.json`);
}
...
/** Day-wise logs root. */
export function getDailyLogsDir() {
  return join(getStateDir(), 'daily-logs');
}

/** One category under the day-wise logs root. */
export function getDailyLogDir(category) {
  return join(getDailyLogsDir(), String(category || 'runtime'));
}

/** Legacy top-level team activity event stream path. */
export function getLegacyTeamActivityLogPath() {
  return join(getStateDir(), 'team-activity.jsonl');
}

/** Team activity event stream (JSONL) for dashboard live feed. */
export function getTeamActivityLogPath(date = new Date()) {
  return join(getDailyLogDir('team-activity'), `${localDateKey(date)}.jsonl`);
}

/** Team registry for project-scoped multi-agent rosters. */
export function getTeamsStorePath() {
  return join(getStateDir(), 'teams.json');
}

/** Structured request / LLM timing events for performance analysis. */
export function getRequestTimingLogPath(date = new Date()) {
  return join(getDailyLogDir('timing'), `${localDateKey(date)}.jsonl`);
}

/** Self-edit audit log produced by System Pulse. */
export function getSelfEditsLogPath(date = new Date()) {
  return join(getDailyLogDir('self-edits'), `${localDateKey(date)}.log`);
}
...
/** Active soft task frames used to keep short follow-up turns scoped. */
export function getTaskFramesStorePath() {
  return join(getStateDir(), 'task-frames.json');
}
