...
function scriptBasenameFromCommand(command) {
  ...
  for (const t of tokens) {
    if (/\.(sh|bash|py|pl|rb|js|mjs|cjs|ts|php|ps1|cmd|bat|command)$/i.test(t) || isAbsolute(t) || t.startsWith('~/') || t.startsWith('~\\')) {
      return basename(t);
    }
  }
  return null;
...
export function deriveCronName(crontabNote, scriptPath, command) {
  ...
  if (path) return humanizeScriptName(basename(path));
  return 'Cron job';
}
...
function getCommandArgs(command, scriptPath) {
  ...
  const idx = tokens.findIndex((t) => t === scriptPath || t.endsWith(scriptPath) || basename(t) === base);
...
export function deriveTechnicalDetails(entry, info = {}) {
  ...
  } else if (path && path !== basename(path)) {
