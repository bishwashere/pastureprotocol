...
export function checkLlmKeys({ envPath } = {}) {
  const id = 'llm-keys';
  const label = 'Cloud LLM API keys';
  ...
  if (populated.length === 0) {
    return {
      ...
      ok: false,
      severity: 'warn',
      message: `No cloud LLM keys configured in ${path}.`,
      hint: 'Set LLM_1_API_KEY (or LLM_2/LLM_3) so the daemon has a usable cloud model. Without one it falls back to a local model that hallucinates tool calls.',
    };
  }
