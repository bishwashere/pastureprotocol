...
function buildStartArgs(indexJs, installDir, dailyLogPath, dailyErrPath) {
...
function startPm2Process(indexJs, installDir, dailyLogPath, dailyErrPath, env) {
  return pm2Args(buildStartArgs(indexJs, installDir, dailyLogPath, dailyErrPath), env);
}
...
export function runPm2DaemonAction(action, opts) {
  ...
  switch (action) {
    case 'start': {
      ...
      const r = startPm2Process(indexJs, installDir, dailyLogPath, dailyErrPath, env);
      if (r.status === 0) {
