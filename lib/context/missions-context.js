/**
 * Inject active Missions into chat turns when the user's message relates to ongoing work.
 * Projects tracker is a lightweight catalog; Missions hold objectives, plan, and tasks.
 */

import { listMissions } from './missions.js';
import { listProjects } from './projects-db.js';
import { pickFocusedProject } from './projects-context.js';
import { BLOCKER_TYPE_PREFIX, inferBlockerType, stripBlockerTitlePrefix } from './tasks.js';
import {
  countAssumptionPhaseSummary,
  formatAssumptionConvertedBlockersForPrompt,
  formatMissionBlockersForPrompt,
  listAssumptionConvertedBlockers,
  listAssumptionPendingBlockers,
  resolveProjectContextForMission,
} from './product-assumptions.js';
import {
  formatTaskHistoryForPrompt,
  listTasksWithHistory,
  TASK_HISTORY_KIND,
} from './task-history.js';

function summarize(text, max = 200) {
  const s = String(text || '').replace(/\s+/g, ' ').trim();
  if (!s) return '';
  return s.length > max ? `${s.slice(0, max - 1)}…` : s;
}

function historyBlob(historyMessages) {
  return (historyMessages || [])
    .slice(-8)
    .map((m) => String(m?.content || ''))
    .join('\n');
}

function combinedUserContext(userText, historyMessages) {
  return `${String(userText || '')}\n${historyBlob(historyMessages)}`.toLowerCase();
}

function normalizeProjectOrMissionIntent(value) {
  return ['discover', 'continue', 'status'].includes(value) ? value : 'none';
}

function missionMatchesText(mission, blob) {
  if (!mission || !blob) return false;
  const title = String(mission.title || '').trim().toLowerCase();
  const objective = String(mission.objective || '').trim().toLowerCase();
  if (title.length >= 3 && blob.includes(title)) return true;
  if (objective.length >= 8 && blob.includes(objective.slice(0, Math.min(40, objective.length)))) return true;
  const words = title.split(/\s+/).filter((w) => w.length >= 4);
  if (words.length && words.every((w) => blob.includes(w))) return true;
  return false;
}

function projectForMission(mission, projects) {
  const pid = Number(mission?.projectId);
  if (Number.isFinite(pid) && pid > 0) {
    const direct = (projects || []).find((p) => Number(p.id) === pid);
    if (direct) return direct;
  }
  const blob = `${mission.title} ${mission.objective}`.toLowerCase();
  return (projects || []).find((p) => {
    const n = String(p.name || '').trim().toLowerCase();
    return n.length >= 3 && blob.includes(n);
  }) || null;
}

/**
 * Pick the Mission the user's message is about (if any).
 *
 * Resolution order — same data the dashboard UI sees (listMissions),
 * no status filtering when the user mentions a mission/project by name:
 *  1. Explicit name/project match against ALL missions (the UI shows all of them).
 *  2. Vague "continue work" fallback — only non-completed missions.
 * @param {{ userText?: string, historyMessages?: Array, agentId?: string, projectOrMissionIntent?: 'none'|'discover'|'continue'|'status' }} opts
 */
export function resolveMissionForUserTurn(opts = {}) {
  const userText = opts.userText || '';
  const historyMessages = opts.historyMessages || [];
  const agentId = String(opts.agentId || 'main').trim() || 'main';
  const projectOrMissionIntent = normalizeProjectOrMissionIntent(opts.projectOrMissionIntent);
  const blob = combinedUserContext(userText, historyMessages);

  let missions = [];
  let projects = [];
  try {
    missions = listMissions().missions || [];
    projects = listProjects();
  } catch (_) {
    return null;
  }
  if (!missions.length) return null;

  const mostRecent = (a, b) => (Number(b.updatedAt) || 0) - (Number(a.updatedAt) || 0);

  // 1. User mentions a project name → match against ALL missions (same as UI).
  const focusedProject = pickFocusedProject(projects, userText, historyMessages);
  if (focusedProject) {
    const pname = String(focusedProject.name || '').trim().toLowerCase();
    const byProject = missions.filter((g) => {
      const gBlob = `${g.title} ${g.objective}`.toLowerCase();
      return pname && (gBlob.includes(pname) || pname.includes(String(g.title || '').trim().toLowerCase()));
    });
    if (byProject.length) {
      byProject.sort(mostRecent);
      return byProject[0];
    }
  }

  // 2. User mentions a mission title/objective → match against ALL missions.
  const byMention = missions.filter((g) => missionMatchesText(g, blob));
  if (byMention.length) {
    byMention.sort(mostRecent);
    return byMention[0];
  }

  // 3. Vague work/discovery/status request — fall back to non-completed missions
  // only when the centralized turn-intent classifier said this turn is about
  // project/mission work.
  if (projectOrMissionIntent !== 'none') {
    const nonCompleted = missions.filter((g) => String(g.status || 'active').toLowerCase() !== 'completed');
    if (!nonCompleted.length) return null;
    const owned = nonCompleted.filter((g) => String(g.ownerAgentId || '').trim() === agentId);
    const running = owned.find((g) => !!g.running);
    if (running) return running;
    if (owned.length === 1) return owned[0];
    if (nonCompleted.length === 1) return nonCompleted[0];
  }

  return null;
}

function formatPlanSteps(mission) {
  const steps = mission?.currentPlan?.steps || [];
  if (!steps.length) return '';
  return steps
    .slice(0, 12)
    .map((s) => `- [${s.status || 'todo'}] ${s.title}`)
    .join('\n');
}

function formatTasksBrief(tasks, depth = 0) {
  if (!Array.isArray(tasks) || depth > 3) return '';
  return tasks
    .slice(0, 10)
    .map((sg) => {
      const indent = '  '.repeat(depth);
      const assignee = sg.assignee ? ` (assignee: ${sg.assignee})` : '';
      const labelStr = Array.isArray(sg.labels) && sg.labels.length
        ? ` [labels: ${sg.labels.join(', ')}]` : '';
      const line = `${indent}- [${sg.status || 'todo'}] ${sg.title}${assignee}${labelStr}`;
      const kids = formatTasksBrief(sg.tasks, depth + 1);
      return kids ? `${line}\n${kids}` : line;
    })
    .join('\n');
}

/** Collect all tasks (recursively) whose status is 'blocked' or whose labels include 'blocker'. */
function collectBlockerTasks(tasks, acc = []) {
  if (!Array.isArray(tasks)) return acc;
  for (const t of tasks) {
    const status = String(t.status || '').toLowerCase();
    const labels = Array.isArray(t.labels) ? t.labels.map((l) => String(l).toLowerCase()) : [];
    if (status === 'blocked' || labels.includes('blocker')) {
      acc.push(t);
    }
    collectBlockerTasks(t.tasks, acc);
  }
  return acc;
}

function formatBlockerTasksList(tasks) {
  const blockers = collectBlockerTasks(tasks).filter((t) => {
    // system_error tasks auto-retry and are never user-actionable — hide from context.
    const bt = String(t.blockerType || inferBlockerType(t.title, t.description)).toLowerCase();
    return bt !== 'system_error';
  });
  if (!blockers.length) return '';
  return blockers
    .map((t) => {
      const assignee = t.assignee ? ` (assignee: ${t.assignee})` : '';
      const desc = t.description ? `\n    ${summarize(t.description, 200)}` : '';
      const title = String(t.title || '').trim();
      if (stripBlockerTitlePrefix(title) !== title) {
        return `- ${title}${assignee}${desc}`;
      }
      const bt = String(t.blockerType || inferBlockerType(title, t.description)).toLowerCase();
      const prefix = BLOCKER_TYPE_PREFIX[bt] || 'Need direction';
      return `- ${prefix}: ${title}${assignee}${desc}`;
    })
    .join('\n');
}

function countTasksByStatus(tasks) {
  const counts = {};
  function walk(list) {
    if (!Array.isArray(list)) return;
    for (const t of list) {
      const s = String(t.status || 'todo').toLowerCase();
      counts[s] = (counts[s] || 0) + 1;
      walk(t.tasks);
    }
  }
  walk(tasks);
  return counts;
}

function formatTaskStatusSummary(tasks) {
  const counts = countTasksByStatus(tasks);
  const total = Object.values(counts).reduce((a, b) => a + b, 0);
  if (!total) return '';
  const parts = Object.entries(counts)
    .sort(([, a], [, b]) => b - a)
    .map(([status, count]) => `${status}: ${count}`);
  return `Total tasks: ${total} — ${parts.join(', ')}`;
}

/**
 * System prompt block for the Mission tied to this user message.
 */
export function buildMissionsContextBlock(opts = {}) {
  try {
    const mission = resolveMissionForUserTurn(opts);
    if (!mission) return '';

    const projects = listProjects();
    const project = projectForMission(mission, projects);
    const projectContext = resolveProjectContextForMission(mission);
    const projectUrl = projectContext.projectUrl || (project?.url ? String(project.url).trim() : '');
    const assumptionSummary = countAssumptionPhaseSummary(mission.tasks || [], projectContext);
    const assumptionPending = listAssumptionPendingBlockers(mission.tasks || [], projectContext);
    const steps = formatPlanSteps(mission);
    const tasks = formatTasksBrief(mission.tasks || []);
    const workRequest = normalizeProjectOrMissionIntent(opts.projectOrMissionIntent) !== 'none';

    const statusLabel = String(mission.status || 'active').toLowerCase();
    const projectName = project ? String(project.name || '').trim() : '';
    const lines = [
      `\n\n# Mission: ${mission.title || mission.id} (${statusLabel})`,
      `The user's message refers to ${projectName ? `project **${projectName}** / ` : ''}mission **${mission.title || mission.id}**. ` +
      `Answer questions about this project/mission ONLY from the structured data below — the same data the dashboard shows. ` +
      `Do NOT pull answers from system logs, daemon errors, background infrastructure status, LLM limits, GitHub errors, ` +
      `delegation chain errors, or any other system-level context that may appear elsewhere in this prompt. ` +
      `Those are unrelated runtime issues, not project or mission data.`,
      `Mission ID: ${mission.id}`,
      `Owner: ${mission.ownerAgentId || 'main'}`,
      `Status: ${mission.status} | Progress: ${mission.progress?.pct ?? 0}%`,
      `Objective: ${mission.objective || mission.title || ''}`,
    ];
    if (project) {
      lines.push(`Related Projects tracker entry: **${project.name}**${projectUrl ? ` (${projectUrl})` : ''}`);
    }
    if (projectContext.hasLiveProduct) {
      lines.push(
        `Live product URL: ${projectUrl}. Assumption phase (last): ${assumptionSummary.pending} pending, ${assumptionSummary.applied} converted to open work — audit on task.blockerHistory + task.assumptionRecord + mission memory.`,
      );
    }
    if (assumptionPending.length) {
      lines.push(
        `Assumption phase pending for: ${assumptionPending.slice(0, 4).map((t) => stripBlockerTitlePrefix(t.title || '')).join('; ')}`,
      );
    }
    const blockersPrompt = formatMissionBlockersForPrompt(mission, projectContext);
    lines.push(blockersPrompt);
    const converted = listAssumptionConvertedBlockers(mission.tasks || []);
    if (converted.length) {
      lines.push(formatAssumptionConvertedBlockersForPrompt(mission.tasks || []));
    }
    const historyTasks = listTasksWithHistory(mission.tasks || [], { kind: TASK_HISTORY_KIND.BLOCKER_CONVERTED });
    if (historyTasks.length) {
      lines.push('Task history — blocker conversions (task.taskHistory on each task):');
      historyTasks.slice(0, 6).forEach((task) => {
        const snippet = formatTaskHistoryForPrompt(task);
        if (snippet) lines.push(`${task.title || task.id}:\n${snippet}`);
      });
    }
    if (mission.lastActivity) lines.push(`Last activity: ${mission.lastActivity}`);
    if (steps) lines.push(`Plan steps:\n${steps}`);
    const statusSummary = formatTaskStatusSummary(mission.tasks || []);
    if (statusSummary) lines.push(statusSummary);
    const blockerTasksList = formatBlockerTasksList(mission.tasks || []);
    if (blockerTasksList) {
      lines.push(`Blocked-status tasks (status=blocked or label=blocker):\n${blockerTasksList}`);
    }
    if (tasks) lines.push(`All tasks:\n${tasks}`);
    if (workRequest) {
      lines.push(
        '',
        '**How to handle this message:**',
        '- Treat this as continuing the mission above — use tools for the next plan step or task, not idle clarification.',
        '- Use the related project URL when relevant; blockers stay documented — run the assumption phase last to attach evidence-backed assumptionRecord data.',
        '- You may run the mission on the next autonomous tick; for chat, answer from what you learn now.',
      );
    }
    return lines.join('\n');
  } catch (err) {
    console.log('[missions-context] failed:', err?.message || err);
    return '';
  }
}

/** Label for agent context currentMission field. */
export function missionLabelForAgentContext(mission) {
  if (!mission) return '';
  return summarize(mission.title || mission.objective, 120);
}

const RESEARCH_SKILL_IDS = ['browse', 'github', 'memory', 'go-read', 'read', 'search'];

/**
 * Build an intent-planner hint from an already-resolved Mission.
 * User intent is decided by turn-intent-classifier.md; this helper only turns
 * mission data + enabled skills into a planner object.
 */
export function buildMissionIntentPlan(mission, enabledSkillIds) {
  if (!mission) return null;
  const skills = (enabledSkillIds || []).filter((id) => RESEARCH_SKILL_IDS.includes(id));
  if (!skills.length) return null;
  const title = String(mission.title || mission.objective || mission.id).trim();
  return {
    mode: 'tool',
    skills,
    plan: `Continue mission "${title}": use tools to advance the mission before asking the user to pick repo/path/source.`,
    answer_style: 'normal',
  };
}
