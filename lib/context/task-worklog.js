...
export const TASK_WORKLOG_MAX_DURABLE_FACTS = 200;
...
/**
 * Keep the structured, LLM-selected facts from every checkpoint even after the
 * verbose checkpoint tail is pruned. This is a mechanical merge of structured
 * checkpoint output; deciding what is important still happens in the
 * tool-result-checkpoint Markdown prompt.
 */
function normalizeDurableResults(raw = {}, checkpoints = []) {
  const source = raw && typeof raw === 'object' ? raw : {};
  const checkpointsNewestFirst = [...checkpoints].reverse();
  const fromCheckpoints = (key) => checkpointsNewestFirst.flatMap((checkpoint) => checkpoint?.[key] || []);
  const latestNextStepState = checkpointsNewestFirst.find((checkpoint) => (
    Array.isArray(checkpoint?.nextSteps) || checkpoint?.nextStep
  ));
  const latestFailureState = checkpointsNewestFirst.find((checkpoint) => Array.isArray(checkpoint?.failures));
  return {
    summaries: cleanList(
      Array.isArray(source.summaries) ? source.summaries : fromCheckpoints('summary'),
      120,
      500,
    ),
    facts: cleanList(
      Array.isArray(source.facts) ? source.facts : fromCheckpoints('facts'),
      TASK_WORKLOG_MAX_DURABLE_FACTS,
      400,
    ),
    completed: cleanList(
      Array.isArray(source.completed) ? source.completed : fromCheckpoints('completed'),
      120,
      300,
    ),
    evidence: cleanList(
      Array.isArray(source.evidence) ? source.evidence : fromCheckpoints('evidence'),
      120,
      400,
    ),
    failures: cleanList(
      Array.isArray(source.failures) ? source.failures : (latestFailureState?.failures || []),
      80,
      400,
    ),
    nextSteps: cleanList(
      Array.isArray(source.nextSteps)
        ? source.nextSteps
        : [
            ...(latestNextStepState?.nextSteps || []),
            ...(latestNextStepState?.nextStep ? [latestNextStepState.nextStep] : []),
          ],
      80,
      300,
    ),
...
function mergeDurableResults(current, checkpoint) {
  const prior = normalizeDurableResults(current);
  const superseded = new Set(checkpoint?.supersedes || []);
  const merge = (key, maxItems, maxChars) => cleanList(
    [
      ...(checkpoint?.[key] || []),
      ...(key === 'nextSteps' && checkpoint?.nextStep ? [checkpoint.nextStep] : []),
      ...prior[key].filter((item) => !superseded.has(item)),
    ],
    maxItems,
    maxChars,
  );
  return {
    summaries: cleanList([
      ...
      ...prior.summaries.filter((item) => !superseded.has(item)),
    ], 120, 500),
    facts: merge('facts', TASK_WORKLOG_MAX_DURABLE_FACTS, 400),
    completed: merge('completed', 120, 300),
    evidence: merge('evidence', 120, 400),
    failures: Array.isArray(checkpoint?.failures)
      ? cleanList(checkpoint.failures, 80, 400)
      : merge('failures', 80, 400),
    // The checkpoint prompt reports current remaining work, so an explicit
    // empty array clears stale steps instead of preserving historical todos.
    nextSteps: Array.isArray(checkpoint?.nextSteps)
      ? cleanList([
          ...checkpoint.nextSteps,
          ...(checkpoint?.nextStep ? [checkpoint.nextStep] : []),
        ], 80, 300)
      : merge('nextSteps', 80, 300),
  };
}
...
function normalizeCheckpoint(input, fallbackSequence = 1) {
  ...
  const hasFailuresState = Array.isArray(row.failures);
  const nextSteps = cleanList(row.nextSteps, 16, 500);
  const hasNextStepsState = Array.isArray(row.nextSteps);
  ...
  return {
    ...
    ...(hasFailuresState || failures.length ? { failures } : {}),
    ...(hasNextStepsState || nextSteps.length ? { nextSteps } : {}),
...
function normalizeWorklog(raw, id, ctx = {}) {
  ...
  return {
    ...
    checkpoints,
    durableResults: normalizeDurableResults(raw.durableResults, checkpoints),
    toolEvents,
...
function newWorklog(id, ctx, metadata = {}) {
  ...
  return {
    ...
    checkpoints: [],
    durableResults: normalizeDurableResults(),
    toolEvents: [],
...
function boundWorklog(log) {
  ...
  log.durableResults = normalizeDurableResults(log.durableResults, log.checkpoints);

  // Match writeJsonAtomic's pretty-printed representation so the on-disk cap
  // is real, not merely a cap on a smaller compact serialization.
  const serializedBytes = () => Buffer.byteLength(JSON.stringify(log, null, 2), 'utf8');
  ...
  // Preserve exact user-relevant facts longest. Other aggregate fields are
  // useful context, but can be reconstructed or summarized if the file hits
  // its strict byte ceiling.
  for (const key of ['evidence', 'summaries', 'completed']) {
    while (bytes > TASK_WORKLOG_MAX_BYTES && log.durableResults[key].length > 0) {
      log.durableResults[key].pop();
      bytes = serializedBytes();
    }
  }
  for (const key of ['nextSteps', 'failures']) {
    while (bytes > TASK_WORKLOG_MAX_BYTES && log.durableResults[key].length > 1) {
      log.durableResults[key].pop();
      bytes = serializedBytes();
    }
  }
  while (bytes > TASK_WORKLOG_MAX_BYTES && log.durableResults.facts.length > 1) {
    log.durableResults.facts.pop();
    bytes = serializedBytes();
  }
...
export function appendTaskWorklogCheckpoint(ctx, checkpoint) {
  ...
  log.durableResults = mergeDurableResults(log.durableResults, normalized);
...
export function formatTaskWorklogPromptBlock(ctxOrWorklog, options = {}) {
  ...
  const durable = normalizeDurableResults(log.durableResults, log.checkpoints);
  const aggregateBudget = Math.floor(maxChars * 0.65);
  const aggregateLine = (label, items, share) => {
    if (!items.length) return '';
    return `${label}: ${redactTaskWorklogText(items.join('; '), Math.max(200, Math.floor(aggregateBudget * share)))}`;
  };
  const durableLines = [];
  // Budget fields independently so verbose summaries can never crowd exact
  // facts or the current blocker/next-step state out of the reinjected prompt.
  durableLines.push(
    aggregateLine('Facts', durable.facts, 0.55),
    aggregateLine('Remaining/next', durable.nextSteps, 0.10),
    aggregateLine('Failures/blockers', durable.failures, 0.10),
    aggregateLine('Summaries', durable.summaries, 0.10),
    aggregateLine('Completed', durable.completed, 0.08),
    aggregateLine('Evidence', durable.evidence, 0.07),
  );
  const presentDurableLines = durableLines.filter(Boolean);
  if (presentDurableLines.length) {
    // Keep the cross-run aggregate bounded so the newest detailed checkpoints
    // still fit in the prompt. The aggregate is deliberately placed first so
    // early facts survive transcript compaction and checkpoint-tail pruning.
    lines.push(
      '',
      'Durable aggregate (deduplicated across all checkpoints):',
      presentDurableLines.join('\n'),
    );
  }
