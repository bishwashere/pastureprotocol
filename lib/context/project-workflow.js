...
import { normalizeTaskLabels, ensureTaskBlockerFields } from './tasks.js';
import { applyTaskPatchHistory } from './task-history.js';
import {
  inferBlockerTemplateTasks,
  formatBlockerTemplatesForPrompt,
} from '../agent/templates/blocker-templates.js';
import {
...
function mergeTemplateBlockerTasks(tasks, input, project) {
  const includeTemplates = input.includeBlockerTemplates === true
    || input.includeBlockerTemplates === 'true'
    || !!(input.blockerTemplateId || input.templateId);
  if (!includeTemplates) {
  ...
  function patchTasks(list) {
    ...
    return list.map((sg) => {
      ...
      if (match) {
        ...
        const patched = applyTaskPatchHistory(sg, { ...sg, status: nextStatus, progress }, {
          ts: Date.now(),
          actor: input.actor || sg.assignee || '',
          note: input.note || '',
          outcome: input.note || `Status → ${nextStatus}`,
        });
        return patched;
      }
...
export function buildProjectWorkflowContextBlock(opts = {}) {
  ...
  return (
    ...
    `   For broad questions that genuinely need missing user context (growth, customers, sales, analytics, physical stores, B2B pipeline, content/audience), pass \`includeBlockerTemplates: true\` to add missing-input tasks with status \`blocked\` and label \`blocker\`. Do not add blocker templates to concrete implementation missions. Available blocker templates: ${formatBlockerTemplatesForPrompt()}.\n` +
