...
import { getLegacyMissionsStorePath, getMissionsStorePath } from '../util/paths.js';
import { readTeamActivityWindow } from '../agent/team-activity.js';
import { createSuggestedTasks, removeSuggestedTasksForMission } from './ai-suggested-tasks.js';
import { formatUserFacingReply } from '../agent/user-facing-reply.js';
import {
  TASK_LABELS,
  addTaskLabel,
...
function normalizeTasks(tasks, opts = {}) {
  ...
  return tasks
    .map((row, idx) => {
      ...
      const assumptionRecord = normalizeAssumptionRecord(row.assumptionRecord || row.assumption_record || null);
      if (assumptionRecord) out.assumptionRecord = assumptionRecord;
      const storedBlockerHistory = row.blockerHistory || row.blocker_history;
      if (storedBlockerHistory) {
        const blockerHistory = normalizeBlockerHistory(storedBlockerHistory, row);
        if (blockerHistory) out.blockerHistory = blockerHistory;
      }
      const taskHistory = normalizeTaskHistory(row.taskHistory || row.task_history || []);
      if (taskHistory.length) out.taskHistory = taskHistory;
      return ensureTaskBlockerFields(out);
    })
    .filter(Boolean)
    .slice(0, 40);
}
...
export function mergeTaskTrees(existing, incoming) {
  ...
  const titleKey = (task) => stripBlockerTitlePrefix(String(task.title || '').trim()).toLowerCase();
  const byTitle = new Map(flatIncoming.map((s) => [titleKey(s), s]));

  function patchList(list) {
    const now = nowMs();
    return (list || []).map((sg) => {
      const match = byId.get(sg.id) || bySlug.get(sg.slug) || byTitle.get(titleKey(sg));
      ...
      let next = match
        ...
        : { ...sg };
      Object.assign(next, ensureTaskBlockerFields({
        ...next,
      ...
      if (match?.assumptionRecord) {
        const normalized = normalizeAssumptionRecord(match.assumptionRecord);
        if (normalized) next.assumptionRecord = normalized;
      }
      if (match?.blockerHistory) {
        const normalizedHistory = normalizeBlockerHistory(match.blockerHistory, next);
        if (normalizedHistory) next.blockerHistory = normalizedHistory;
      }
      if (match) {
        next = applyTaskPatchHistory(sg, next, {
          ts: now,
          actor: match.assignee || next.assignee || sg.assignee || '',
          source: 'mission_tick',
        });
      }
...
function dedupTitleKey(title) {
  return String(stripBlockerTitlePrefix(title) || '')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, '')
    .replace(/\s+/g, ' ')
...
function blockedTaskTitles(rawTasks, normTasks) {
  const titles = [];
  function walk(rawList, normList) {
    for (let i = 0; i < (normList || []).length; i++) {
      const norm = normList[i];
      const raw = (rawList || [])[i];
      if (String(norm?.status || '').toLowerCase() === 'blocked') {
        titles.push({
          raw: String(raw?.title || '').trim(),
          norm: String(norm?.title || '').trim(),
        });
      }
      walk(raw?.tasks || raw?.subgoals || [], norm?.tasks || []);
    }
  }
  walk(rawTasks, normTasks);
  return titles;
}

function missionsBlockerTitlesChanged(rawRows, normalized) {
  for (let i = 0; i < normalized.length; i++) {
    const pairs = blockedTaskTitles(rawRows[i]?.tasks || rawRows[i]?.subgoals || [], normalized[i]?.tasks || []);
    if (pairs.some((p) => p.raw !== p.norm)) return true;
  }
  return false;
}
...
function readStore() {
  ...
  try {
    ...
    if (store.missions.length && (activePath === legacyPath || missionsBlockerTitlesChanged(rows, store.missions))) {
      writeStore(store);
    }
    return store;
  } catch {
...
/**
 * True when at least one mission is persisted on disk.
 *
 * Used as the gate for autonomy loops (mission engine + system pulse).
 * Per the architecture: autonomy only runs once the user has at least
 * one durable mission to maintain — until then, the agent stays a
 * single-shot, request/response single agent.
 *
 * @returns {boolean}
 */
export function hasAnyMission() {
  const store = readStore();
  return Array.isArray(store.missions) && store.missions.length > 0;
}

let onFirstMissionCreatedCb = null;

/**
 * Register a callback that runs after a mission is created. Used by the
 * autonomy gate (lib/agent/autonomy-gate.js) to start the mission engine
 * and system pulse the first time the store goes from empty to non-empty.
 *
 * The callback is responsible for being idempotent — it may be invoked on
 * every subsequent createMission(), not just the first one.
 *
 * @param {((event: { missionId: string, isFirst: boolean }) => void) | null} cb
 */
export function setOnMissionCreated(cb) {
  onFirstMissionCreatedCb = typeof cb === 'function' ? cb : null;
}
...
export function createMission(input = {}) {
  ...
  const store = readStore();
  const wasEmpty = store.missions.length === 0;
  store.missions.push(mission);
  writeStore(store);
  ensureMissionMemoryFile(mission.id, mission.title);
  if (onFirstMissionCreatedCb) {
    try {
      onFirstMissionCreatedCb({ missionId: mission.id, isFirst: wasEmpty });
    } catch (err) {
      console.log('[missions] onMissionCreated hook failed:', err?.message || err);
    }
  }
  return mission;
}
 ...
 * The daily team-activity inbox/outbox logs are intentionally left intact.
 */
...
function unblockTasksAfterUserResponse(tasks) {
  const reopened = (tasks || []).map((sg) => {
    const status = String(sg.status || 'todo').toLowerCase();
    if (status !== 'blocked') {
      return { ...sg, tasks: unblockTasksAfterUserResponse(sg.tasks || []) };
    }
    const next = {
      ...sg,
      status: 'todo',
      title: stripBlockerTitlePrefix(sg.title || ''),
      tasks: unblockTasksAfterUserResponse(sg.tasks || []),
    };
    delete next.blockerType;
    return next;
  });
  return normalizeTasks(reopened);
}
...
export function buildMissionTickPrompt(mission, opts = {}) {
  ...
  const projectContext = opts.projectContext || resolveProjectContextForMission(g);
  const assumptionPending = listAssumptionPendingBlockers(g.tasks || [], projectContext);
  const assumptionPhaseBlock = formatAssumptionPhasePromptBlock(g, projectContext, assumptionPending);
  ...
  return [
    ...
    '- Create blockers normally (blockerType + prefixed title) so they stay documented when someone asks what is blocked.',
    ...
    '  "assumptionUpdates": [{"taskId":"task-id", "assumptionRecord":{"status":"applied","sourceUrl":"https://...", "method":"live_product_browse", "collectedEvidence":["..."], "assumptions":[{"item":"...", "rationale":"...", "confidence":0.8}], "summary":"..."}, "status":"in_progress", "progress":50, "description":"optional task note"}],',
    ...
    '  //     "need_direction"  — strategic/product decision only the owner can make; may later get an assumptionRecord from phase 10.',
    ...
    '  //   Blocked task titles MUST start with the matching prefix, e.g. "Need access: Stripe read-only key".',
    '  //   Tasks without a valid blockerType + prefixed title are rejected/normalized on save.',
...
export async function runMissionTick(missionId, opts = {}) {
  ...
  try {
    ...
    const projectContext = resolveProjectContextForMission(started);
    const assumptionAppliedTasks = applyAssumptionUpdates(
      createResult.tasks,
      Array.isArray(parsed.assumptionUpdates) ? parsed.assumptionUpdates : [],
      projectContext,
    );
    const nextTasks = applyWaitBlocksToTasks(assumptionAppliedTasks, nextWaitCondition);
    ...
    const assumptionUpdates = Array.isArray(parsed.assumptionUpdates) ? parsed.assumptionUpdates : [];
    ...
    const memoryLines = [
      ...
      assumptionUpdates.length
        ? `Assumption conversions: ${flattenMissionTasks(nextTasks)
          .map(formatAssumptionConversionLogLine)
          .filter(Boolean)
          .slice(0, 4)
          .join(' | ') || assumptionUpdates.slice(0, 4).map((row) => {
            const id = String(row.taskId || row.id || '').trim();
            const summary = summarize(row.assumptionRecord?.summary || row.summary || '', 120);
