...
import { formatTaskWorklogPromptBlock } from './task-worklog.js';
...
function normalizeFrame(raw, logKey = '') {
  ...
  return {
    ...
    ownerAgentId: clean(raw.ownerAgentId, 80),
    teamId: clean(raw.teamId, 120),
    ...
    needsWorklog: raw.needsWorklog === true,
    worklogId: clean(raw.worklogId, 100),
...
export function upsertTaskFrame(logKey, decision, existingFrame = null, meta = {}) {
  ...
  const frame = normalizeFrame({
    ...
    ownerAgentId: decision.ownerAgentId || existing?.ownerAgentId,
    teamId: decision.teamId || existing?.teamId,
    toolProfile: decision.toolProfile?.length ? decision.toolProfile : existing?.toolProfile,
    plan: decision.plan || existing?.plan,
    needsWorklog: decision.needsWorklog === true || existing?.needsWorklog === true,
    worklogId: meta.worklogId || existing?.worklogId,
...
export function updateTaskFrameAfterTurn(logKey, patch = {}) {
  ...
  const frame = normalizeFrame({
    ...
    worklogId: patch.worklogId || existing.worklogId,
    updatedAtMs: nowMs(),
  }, key);
  store.frames[key] = frame;
  writeStore(store);
...
export async function classifyTaskFrameTurn({
  ...
  const activeFrameForLlm = activeFrame
    ? { ...activeFrame, worklogId: undefined, hasWorklog: Boolean(activeFrame.worklogId) }
    : null;
...
export function taskFrameDecisionToTurnRoute(decision, frame) {
  ...
  return {
    ...
    skills: Array.isArray(decision.toolProfile) && decision.toolProfile.length > 0
      ? decision.toolProfile
      : (Array.isArray(frame.toolProfile) ? frame.toolProfile : []),
    requiredToolSteps: [],
    executionMode: 'tool_use',
    usesExistingWorkIntake: true,
    mustUseTool: decision.mustUseTool === true,
    needsWorklog: decision.needsWorklog === true || frame.needsWorklog === true,
...
export async function classifyTaskFrameStatusAfterTurn({
  ...
  const frameForLlm = { ...frame, worklogId: undefined, hasWorklog: Boolean(frame.worklogId) };
  const result = await runMdPrompt({
    ...
    user: {
      ...
      worklog: frame.worklogId
        ? formatTaskWorklogPromptBlock({
            taskWorklogId: frame.worklogId,
            agentId,
            logKey: frame.logKey,
          })
        : '',
...
export function taskFrameToSystemBlock(frame, decision = null) {
  ...
  const lines = [
    ...
    frame.ownerAgentId ? `Owner agent: ${frame.ownerAgentId}` : '',
    frame.teamId ? `Team: ${frame.teamId}` : '',
    ...
    'If the frame is blocked or recent chat mentioned missing access, treat that as historical context. When write-capable tools are available in the current turn, attempt the requested write/edit before claiming read-only access or a permission blocker.',
    '---',
  ].filter(Boolean);
  return lines.join('\n');
}
