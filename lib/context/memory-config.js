...
import { getConfigPath, getWorkspaceDir, getMemoryIndexPath } from '../util/paths.js';
import { normalizeLlmAuth, readLlmAuthToken, resolveBearerToken } from '../llm/auth.js';
...
export function getOpenAIKey(raw) {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    if (m && String(m.provider || '').toLowerCase() === 'openai') {
      const auth = normalizeLlmAuth(m, i);
      let key = null;
      if (auth.type === 'api_key') key = auth.env ? fromEnv(auth.env) : fromEnv(auth.value);
      else if (auth.type === 'bearer_token') key = resolveBearerToken(auth);
      else if (auth.type === 'oauth') {
        const token = readLlmAuthToken(auth.cache || auth.account || 'openai');
        key = token?.access_token || token?.accessToken || null;
      } else {
        key = fromEnv(m.apiKey);
      }
 ...
 * @param {{ workspaceDir?: string, indexPath?: string }} [opts]
 * @returns {{
 *   workspaceDir: string,
 *   indexPath: string,
 ...
 * } | null}
 */
export function getMemoryConfig(config = null, opts = {}) {
  ...
  const workspaceOverride = opts?.workspaceDir && String(opts.workspaceDir).trim();
  const workspaceDir = workspaceOverride || (memory.workspaceDir && String(memory.workspaceDir).trim()) || getWorkspaceDir();
  const indexPath = (opts?.indexPath && String(opts.indexPath).trim())
    || (workspaceOverride ? join(workspaceOverride, '.memory', 'index.db') : '')
    || (memory.indexPath && String(memory.indexPath).trim())
    || getMemoryIndexPath();
