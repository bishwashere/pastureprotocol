...
/**
 * Last raw private exchange for this JID, ignoring session boundaries.
 * Used by proactive systems that must know whether the latest activity was
 * user-originated or generated by the system itself.
 * @param {string} workspaceDir
 * @param {string} jid
 * @returns {{ ts: number, sessionId?: string, user: string, assistant: string } | null}
 */
export function getLastPrivateExchange(workspaceDir, jid) {
  if (!workspaceDir || typeof workspaceDir !== 'string' || jid == null || String(jid).trim() === '') return null;
  const perJidPath = resolvePrivateLogFilePath(workspaceDir, jid);
  if (!existsSync(perJidPath)) return null;
  try {
