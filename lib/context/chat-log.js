...
import { appendFileSync, mkdirSync, existsSync, readFileSync, readdirSync, statSync, writeFileSync, unlinkSync } from 'fs';
...
export function appendExchange(workspaceDir, exchange) {
  ...
  const fileName = safeJidForFile(logJid) + '.jsonl';
      ...
      for (const line of raw.split('\n')) {
        ...
        try {
          ...
          const logJid = row.jid != null && String(row.jid).trim() !== ''
            ? String(row.jid).trim()
            : 'unknown';
          const dest = join(privateDir, safeJidForFile(logJid) + '.jsonl');
