...
function migrateProjectsSchema(db) {
  ...
  if (!cols.includes('team_id')) {
    db.exec("ALTER TABLE projects ADD COLUMN team_id TEXT NOT NULL DEFAULT ''");
  }
}
...
export function normalizeProjectTeamId(teamId) {
...
export function createProject({ name, description = '', url = '', setup_notes = '', connectors = null, team_id = '' }) {
  const db = getProjectsDb();
  const now = Date.now();
  ...
  const result = db.prepare(
    'INSERT INTO projects (name, url, description, setup_notes, connectors_json, team_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  ).run(name, normalizeProjectUrl(url), description, String(setup_notes || ''), connectorsJson, normalizeProjectTeamId(team_id), now, now);
  return getProject(result.lastInsertRowid);
}

export function updateProject(id, { name, description, url, setup_notes, connectors, team_id }) {
  const db = getProjectsDb();
  const now = Date.now();
  ...
  if (connectors !== undefined) {
    const current = parseProjectConnectors(existing.connectors_json);
    const incoming = connectors && typeof connectors === 'object' && !Array.isArray(connectors) ? connectors : {};
  ...
  const nextTeamId = team_id !== undefined ? normalizeProjectTeamId(team_id) : (existing.team_id || '');
  db.prepare('UPDATE projects SET name=?, url=?, description=?, setup_notes=?, connectors_json=?, team_id=?, updated_at=? WHERE id=?')
    .run(nextName, nextUrl, nextDescription, nextSetup, nextConnectorsJson, nextTeamId, now, id);
...
export function getProjectGraph(project_id) {
  ...
  return { project: mapProjectRow(project), updates, branches };
}
