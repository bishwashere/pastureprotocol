...
import { isLikelyContinuationReply } from './conversation-context.js';
...
export async function runAgentTurn({
  ...
  const skipCompletenessProbe = isLikelyContinuationReply(userText, historyMessages);
  if (skillsCalled.length > 0 && useTools && stripThinking(finalContent).trim() && !skipCompletenessProbe) {
    for (let cr = 0; cr < MAX_COMPLETENESS_RETRIES; cr++) {
      ...
      messages.push({
        role: 'user',
        content: `You didn't answer all parts of the question. Missing: "${probeMissing}". Use your tools to answer ALL of them now in one response.`,
      });
