...
export function buildMissionsContextBlock(opts = {}) {
  try {
    ...
    if (projectContext.hasLiveProduct) {
      lines.push(
        `Live product URL: ${projectUrl}. Assumption phase (last): ${assumptionSummary.pending} pending, ${assumptionSummary.applied} applied — blockers stay documented; assumptions attach as assumptionRecord on each task.`,
      );
    }
