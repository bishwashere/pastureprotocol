...
export function buildMissionsContextBlock(opts = {}) {
  try {
    ...
    if (blockerTasksList) {
      lines.push(
        `Blocker tasks (status=blocked or label=blocker — these are the ONLY blockers for this mission; do NOT source blockers from system logs, daemon errors, or background infrastructure issues):\n${blockerTasksList}`,
      );
    } else {
      lines.push('Blocker tasks: none (no tasks with status=blocked or label=blocker).');
    }
