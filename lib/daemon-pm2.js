...
function pm2Describe(env) {
  return spawnSync('pm2', ['describe', PM2_NAME, '--no-color'], {
    encoding: 'utf8',
    env,
    shell: process.platform === 'win32',
  });
...
/** Append a control line to daemon.log (same as daemon.sh). */
export function daemonLog(stateDir, action) {
  try {
    mkdirSync(stateDir, { recursive: true });
    const stamp = new Date().toISOString().replace(/\.\d{3}Z$/, '');
    appendFileSync(join(stateDir, 'daemon.log'), `[${stamp}] cowcode ${action}\n`, 'utf8');
  } catch (_) {}
}
...
export function ensurePm2() {
  ...
  console.log('pm2 not found. It is required to manage the cowCode daemon on Windows.');
...
export function runPm2DaemonAction(action, opts) {
  ...
  mkdirSync(stateDir, { recursive: true });
  try {
    appendFileSync(join(stateDir, 'daemon.log'), '', 'utf8');
    appendFileSync(join(stateDir, 'daemon.err'), '', 'utf8');
  } catch (_) {}

  const env = {
    ...process.env,
    COWCODE_INSTALL_DIR: installDir,
  ...
  switch (action) {
    case 'start': {
      ...
      if (desc.status === 0 && desc.stdout?.includes('status')) {
        console.log('Daemon is already running. Logs: pm2 logs cowcode');
        daemonLog(stateDir, 'start');
        return 0;
      }
      const r = pm2Args(['start', indexJs, '--name', PM2_NAME, '--cwd', installDir], env);
      if (r.status === 0) {
        console.log('Started with pm2. To see logs: cowcode logs');
        daemonLog(stateDir, 'start');
      }
      return r.status ?? 1;
    }
    ...
    default:
      console.error('Usage: cowcode start|stop|status|restart');
      return 1;
  }
}
