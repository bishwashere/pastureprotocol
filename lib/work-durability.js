...
async function decomposePersistentWorkWithAi(opts = {}) {
  ...
  const fallback = buildMissionSubgoals(userText, opts.agentId || 'main', opts.deliverables || []);
...
export async function classifyWorkDurabilityWithAi(opts = {}) {
  ...
  try {
    ...
    if (normalized?.persistence === 'create_lightweight_mission') {
      normalized = {
        ...normalized,
        subgoals: await decomposePersistentWorkWithAi({
          ...opts,
...
function routeReasonForSubgoal(sg) {
...
export function buildDurableDelegationContext(decision, {
  ...
  const subgoals = Array.isArray(decision.subgoals) && decision.subgoals.length
    ? decision.subgoals
    : (Array.isArray(decision.goal?.subgoals) ? decision.goal.subgoals : []);
  const routes = subgoals
    .map((sg) => {
      ...
      return {
        ...
        agent: target,
        confidence,
        reason: routeReasonForSubgoal(sg),
        type: clean(sg.type || inferSubtaskType(sg.title), 40),
        subgoalId: clean(sg.id || '', 80),
      };
    })
  ...
  return {
    candidates,
    teamCapability: {
      request: decision.goal?.objective || decision.title || '',
      callerAgentId,
      agents: [
        {
