...
function looksLikeNewMissionCandidate(userText) {
  ...
  // Primary signal: the user mentions a known project by name.
  if (mentionsKnownProject(userText)) return true;
  // Fallback: multi-deliverable messages with explicit launch/build/plan verbs.
...
export async function classifyWorkDurabilityWithAi(opts = {}) {
  ...
  // If the deterministic rules already identified this as a new mission candidate
  // (e.g. the user mentioned a known tracked project), trust that signal and skip
  // the AI classifier — the AI often downgrades project inquiries to direct_answer
  // because it cannot see the local project catalog.
