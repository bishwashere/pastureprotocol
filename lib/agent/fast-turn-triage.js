...
function stripPasturePrefix(text) {
  ...
  for (const prefix of ['[Pasture]', '[Pasture Protocol]']) {
    if (s.startsWith(prefix)) return s.slice(prefix.length).trim();
  }
  return s;
}

function isGeneratedWorkflowFailure(text) {
  const s = stripPasturePrefix(text);
  return s.startsWith('I could not finish the required tool workflow.')
    || s.startsWith('I ran out of tool rounds before finishing this.');
}

function historyWithoutGeneratedFailures(historyMessages = []) {
  ...
  for (const msg of historyMessages) {
    ...
    if (msg?.role !== 'assistant' || pendingUser == null) continue;
    if (!isGeneratedWorkflowFailure(msg.content)) {
      out.push(pendingUser, { role: 'assistant', content: msg.content });
    }
    pendingUser = null;
  }
  return out;
...
export async function planFastTurnTriage({
  ...
  const result = await runMdPrompt({
    ...
    user: {
      ...
      recentConversation: formatHistoryForClassifier(historyWithoutGeneratedFailures(historyMessages), 4),
