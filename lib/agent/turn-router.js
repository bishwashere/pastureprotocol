/**
 * Lightweight turn router — called once per turn BEFORE getSkillContext().
...
import { formatRequiredToolStep, normalizeRequiredToolSteps } from './execution-requirements.js';
...
export function buildCasualChatTurnRoute() {
  return {
    mode: 'chat',
    needsWorklog: false,
    skills: [],
 ...
 */
export async function routeTurn({ userText, historyMessages = [], availableSkillIds = [], availableSkillSummaries, agentId, delegationContext = null, workDurability = null, llmChat: injectedLlmChat = null }) {
  if (isNonTaskMessage(userText)) return buildCasualChatTurnRoute();
  ...
  const durabilitySection = (() => {
    ...
    const lines = [
      ...
      durable.missionId ? `- missionId: ${durable.missionId}` : '',
  ...
  const delegationSection = (() => {
    ...
    if (recommendation?.action === 'delegate' && recommendation?.targetAgentId) {
      ...
      if (recommendation.offerUpgrade) {
        lines.push('Offer the user: handle as coordinator now, or create a dedicated specialist agent / autonomous Mission for long-term work.');
      }
  ...
  const userPrompt =
    ...
    `3. Which skill IDs from the list above are relevant? Use the smallest useful set; code/file implementation may need up to 6. (empty array if none)\n` +
    ...
    `6. Does this require a checkpoint worklog because it has several meaningful tool rounds or many independent results?\n\n` +
    `Return JSON only:\n` +
    `{\n` +
    `  "mode": "chat | tool | research | code | memory",\n` +
    `  "needsWorklog": false,\n` +
    `  "skills": [],\n` +
    `  "requiredToolSteps": [{"kind":"inspect | write | execute | verify | delegate","anyOfSkills":[],"anyOfTools":[],"requiredArguments":{},"resultContains":""}],\n` +
  ...
  const plan = await runMdPrompt({
    promptName: 'turn-router',
    userText: userPrompt,
    agentId,
    purpose: 'turn_router',
  ...
  try {
    ...
    plan.skills = Array.isArray(plan.skills)
      ...
      : [];
    plan.needsWorklog = plan.needsWorklog === true;
    if (plan.needsWorklog && allIds.includes('worklog') && !plan.skills.includes('worklog')) {
      plan.skills.push('worklog');
    }
    const requestedToolSteps = normalizeRequiredToolSteps(plan.requiredToolSteps, allIds);
    for (const step of requestedToolSteps) {
      for (const id of step.anyOfSkills) {
        if (!plan.skills.includes(id)) plan.skills.push(id);
      }
    }
    plan.requiredToolSteps = normalizeRequiredToolSteps(requestedToolSteps, plan.skills);
    const codeOutcomeNeedsTools = plan.mode === 'code'
      && plan.skills.some((id) => ['write', 'edit', 'apply-patch', 'go-write', 'exec'].includes(id));
    plan.mustUseTool = plan.mustUseTool === true || plan.requiredToolSteps.length > 0 || codeOutcomeNeedsTools;
    if (durable) {
    ...
    console.log('[turn-router] post-validation failed, skipping (full tool list will be used):', err.message);
    return null;
  }
}
 ...
 * @returns {string}
 */
export function turnRouteToSystemBlock(plan) {
  if (!plan) return '';
  const requiredSteps = !plan.needsWorklog && Array.isArray(plan.requiredToolSteps)
    ? plan.requiredToolSteps
      .map(formatRequiredToolStep)
  ...
  const lines = [
    ...
    plan.needsWorklog ? 'Durable checkpoint worklog: required for this turn; checkpoint meaningful tool results and read it before final synthesis.' : null,
    plan.mustUseTool ? 'Tool use is mandatory: call at least one planned tool before answering.' : null,
    requiredSteps.length ? `Required tool steps (ordered): ${requiredSteps.join(' -> ')}` : null,
