import { appendFileSync, existsSync, mkdirSync, readdirSync, readFileSync, symlinkSync, unlinkSync, writeFileSync } from 'fs';
import { basename, dirname, join } from 'path';
import {
  getDailyLogDir,
  getLegacyTeamActivityLogPath,
  getTeamActivityLogPath,
} from '../util/paths.js';
...
function refreshCurrentActivityLink(dir, targetName) {
  try {
    const linkPath = join(dir, 'current.jsonl');
    try {
      unlinkSync(linkPath);
    } catch (err) {
    ...
    // The dated JSONL file remains the source of truth.
  }
}

function listTeamActivityLogPaths({ includeLegacy = true } = {}) {
  const paths = [];
  try {
    const legacy = getLegacyTeamActivityLogPath();
    if (includeLegacy && existsSync(legacy)) paths.push(legacy);
  } catch (_) {}
  try {
    const dir = getDailyLogDir('team-activity');
    if (!existsSync(dir)) return paths;
    const daily = readdirSync(dir)
      .filter((name) => name.endsWith('.jsonl') && name !== 'current.jsonl')
      .sort()
...
function readTeamActivityLogText(maxBytes) {
  const paths = listTeamActivityLogPaths();
  ...
  for (const path of paths) {
    try {
      ...
      if (!raw) continue;
      out += raw.endsWith('\n') ? raw : raw + '\n';
      if (out.length > maxBytes * 2) out = trimForTail(out, maxBytes * 2);
    } catch (_) {}
  }
  return trimForTail(out, maxBytes);
}
...
export function readTeamActivityWindow({ maxBytes = MAX_METRICS_READ_BYTES, maxEvents = MAX_METRICS_EVENTS } = {}) {
  ...
  try {
    const tail = readTeamActivityLogText(byteLimit);
    if (!tail) return [];
...
export function logTeamActivity(event = {}) {
  try {
    ...
    refreshCurrentActivityLink(dir, basename(logPath));
    const now = Date.now();
    const row = {
...
export function readTeamActivity({ since = 0, limit = 80 } = {}) {
  ...
  try {
    const tail = readTeamActivityLogText(MAX_READ_BYTES);
    if (!tail) return [];
    const lines = tail.split('\n').filter((l) => l.trim());
    const out = [];
...
export function pruneTeamActivityForMission(missionId) {
  ...
  const paths = listTeamActivityLogPaths();
  if (!paths.length) return { kept: 0, pruned: 0 };
  let totalKept = 0;
  let totalPruned = 0;
  ...
  return { kept: totalKept, pruned: totalPruned };
}
