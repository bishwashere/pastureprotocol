...
import { getAgentContextStatePath } from '../util/paths.js';
import { parseInternalPairJid, extractProjectContext } from './team-inbox.js';
import { getAgentTitle } from './agent-config.js';
import { resolveMissionForUserTurn, missionLabelForAgentContext } from '../context/missions-context.js';
import { listMissions } from '../context/missions.js';
...
/**
 * Clear currentMission on any agent whose active mission label matches the deleted mission's title.
 * Called after a mission is deleted so agents don't keep displaying a stale mission label.
 */
export function clearMissionFromAgentContext(missionTitle) {
  const title = String(missionTitle || '').trim();
  if (!title) return;
  const store = readStore();
  let changed = false;
  for (const [agentId, row] of Object.entries(store.agents || {})) {
    const current = String(row.currentMission || '').trim();
...
export function onAgentTurnDone({ agentId, status = 'ok' } = {}) {
  const id = String(agentId || '').trim();
  const prev = readAgentContext(id);
  const isError = status === 'error';
  const lastAction = prev?.lastAction
    ? summarizeTask(
        `${isError ? 'Turn ended with errors' : 'Completed turn'} — ${prev.lastAction}`,
        140
      )
    : isError
