...
// Control-plane helpers preserve execution state but are never evidence that
// the user's live inspect/write/execute/verify request actually ran.
const CONTROL_ONLY_SKILL_IDS = new Set(['worklog']);
...
export function normalizeRequiredToolSteps(rawSteps, availableSkillIds = null) {
  ...
  for (const raw of Array.isArray(rawSteps) ? rawSteps : []) {
    ...
    const anyOfSkills = uniqueSkillIds(
      ...
      8,
    ).filter((id) => !CONTROL_ONLY_SKILL_IDS.has(id));
...
export function buildExecutionRequirements(turnRoute) {
  ...
  const worklogDecisionMade = Object.prototype.hasOwnProperty.call(route, 'needsWorklog');
  const worklogRequired = route.needsWorklog === true;
  const plannedSkills = uniqueSkillIds(route.skills, null, 40);
  const executionSkills = plannedSkills.filter((id) => !CONTROL_ONLY_SKILL_IDS.has(id));
  let steps = worklogRequired
    ? []
    : normalizeRequiredToolSteps(route.requiredToolSteps, plannedSkills);
  const source = worklogRequired ? 'worklog' : (steps.length > 0 ? 'planner' : 'fallback');

  if (!worklogRequired && steps.length === 0 && route.mustUseTool === true && executionSkills.length > 0) {
    steps.push({
      kind: 'inspect',
      anyOfSkills: executionSkills,
      anyOfTools: [],
      requiredArguments: {},
      resultContains: '',
    });
  ...
  return {
    ...
    worklogRequired,
    worklogDecisionMade,
    plan: typeof route.plan === 'string' ? route.plan.slice(0, 1600) : '',
  };
}
...
export function toolCallMatchesRequiredStep(step, toolCall, { checkResult = true } = {}) {
  ...
  if (CONTROL_ONLY_SKILL_IDS.has(skillId)) return false;
