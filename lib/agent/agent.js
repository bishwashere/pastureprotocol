...
function extractToolErrorMessage(result) {
  return parseSkillResult(result).error || '';
}
...
export async function runAgentTurn({
  ...
  const writeIntent = parseWriteIntent(userText);
  const forcedWrite = hasSkillEnabled(toolsToUse, 'write') ? writeIntent : null;
  if (writeIntent && !forcedWrite) {
    finishTurnState('ok', `Handled in ${Date.now() - turnStartedAt}ms (write-denied).`);
    return {
      textToSend: "[Pasture] I don't have the write skill assigned, so I can't create files directly. Ask an admin to grant the write skill to this agent, or paste the content into a file manually.",
      skillsCalled: [],
    };
  }
  if (forcedWrite) {
    ...
    try {
      ...
      if (parsed?.error) {
        body = toUserMessage(parsed.error);
        writeOk = false;
      } else body = `Wrote ${parsed.path || forcedWrite.path} with the exact content you provided.`;
    } catch (_) {}
    finishTurnState(
      writeOk ? 'ok' : 'error',
      `Handled in ${Date.now() - turnStartedAt}ms (write${writeOk ? '' : ' failed'}).`
    );
    return { textToSend: stripAsterisks('[Pasture] ' + body), skillsCalled: ['write'] };
  }
  const editIntent = parseEditIntent(userText);
  const forcedEdit = hasSkillEnabled(toolsToUse, 'edit') ? editIntent : null;
  if (editIntent && !forcedEdit) {
    finishTurnState('ok', `Handled in ${Date.now() - turnStartedAt}ms (edit-denied).`);
    return {
      textToSend: "[Pasture] I don't have the edit skill assigned, so I can't modify files directly. Ask an admin to grant the edit skill to this agent.",
      skillsCalled: [],
    };
  }
  if (forcedEdit) {
    ...
    finishTurnState(
      editOk ? 'ok' : 'error',
      `Handled in ${Date.now() - turnStartedAt}ms (edit${editOk ? '' : ' failed'}).`
    );
  ...
  if (forcedHa) {
    ...
    finishTurnState(
      haOk ? 'ok' : 'error',
      `Handled in ${Date.now() - turnStartedAt}ms (home-assistant${haOk ? '' : ' failed'}).`
    );
  ...
  // Audit finding #20: previously this just appended a verification user
  // message into `messages` and depended on a downstream synthesis path that
  // would only fire when finalContent was empty. As a result, write turns
  // could report "Done. Wrote 3 files" while disk showed none of them.
  // Fix: when verification produced any output, ALWAYS run one no-tools
  // synthesis pass and let it ground/correct finalContent.
    ...
    try {
      ...
      for (const dir of writtenDirs) {
        try {
          ...
          if (lsResult && parseSkillResult(lsResult).ok) {
  ...
  if (useTools && !stripThinking(finalContent).trim() && lastRoundHadToolError) {
    try {
      const { content: clarification } = await chatWithTools(messages, [], agentLlmOptions('agent_turn_tool_error_clarify'));
  ...
  if (searchResult && !stripThinking(finalContent).trim()) {
    try {
      const synthesized = await chatWithTools(messages, [], agentLlmOptions('agent_turn_search_synthesis'));
      const reply = synthesized?.content && stripThinking(synthesized.content).trim();
      if (reply) finalContent = reply;
    } catch (_) {}
  }
  if (browseResult && !stripThinking(finalContent).trim()) {
    try {
      const synthesized = await chatWithTools(messages, [], agentLlmOptions('agent_turn_browse_synthesis'));
      const reply = synthesized?.content && stripThinking(synthesized.content).trim();
      if (reply) finalContent = reply;
    } catch (_) {}
  }
  if (visionResult && !stripThinking(finalContent).trim()) {
    try {
      const synthesized = await chatWithTools(messages, [], agentLlmOptions('agent_turn_vision_synthesis'));
      const reply = synthesized?.content && stripThinking(synthesized.content).trim();
      if (reply) finalContent = reply;
    } catch (_) {}
  }
  ...
  if (lastToolResult && (!stripThinking(finalContent).trim() || looksLikeBrushOff(finalContent))) {
    try {
      const { content: synthesized } = await chatWithTools(messages, [], agentLlmOptions('agent_turn_tool_result_synthesis'));
    ...
    for (let cr = 0; cr < MAX_COMPLETENESS_RETRIES; cr++) {
      ...
      try {
        ...
        if (retryResp.toolCalls?.length) {
          ...
          const synthesis = await chatWithTools(messages, [], agentLlmOptions(`agent_turn_completeness_synthesis_r${cr}`));
  ...
  const turnStatus = lastRoundHadToolError || roundsExhausted ? 'error' : 'ok';
  logTeamActivity({
    type: 'turn_done',
    agentId: currentAgentId,
    depth: Number.isFinite(ctx?.agentDepth) ? ctx.agentDepth : 0,
