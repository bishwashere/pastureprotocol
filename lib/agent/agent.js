  ...
  if (hadWriteOp && writtenDirs.size > 0) {
    try {
      ...
      if (verifyParts.length > 0) {
        ...
        // the user-facing reply is grounded in the actual disk state.
        try {
      ...
      try {
        ...
        if (retryResp.toolCalls?.length) {
          ...
          for (const tc of retryResp.toolCalls) {
            ...
            // Track write operations the same way the main loop does so the
            // post-write verification block can see them too.
            if (!tcRetryIsError && isWriteToolCall(tcSkillId, tc.name)) {
              hadWriteOp = true;
              const d = extractWrittenDir(tcSkillId, tcRunArgs);
              if (d) writtenDirs.add(d);
            }
