...
import { logTiming } from './request-timing.js';
...
export async function runAgentTurn({
  ...
  if (writeIntent && !forcedWrite) {
    // Write intent detected but the write skill is not assigned to this agent.
    logTeamActivity({
      type: 'turn_done',
      agentId: currentAgentId,
      depth: Number.isFinite(ctx?.agentDepth) ? ctx.agentDepth : 0,
      jid: ctx?.jid || '',
      status: 'ok',
      message: `Handled in ${Date.now() - turnStartedAt}ms (write-denied).`,
    });
    return {
  ...
  if (editIntent && !forcedEdit) {
    logTeamActivity({
      ...
      jid: ctx?.jid || '',
      status: 'ok',
      message: `Handled in ${Date.now() - turnStartedAt}ms (edit-denied).`,
    });
    return {
