...
export async function runAgentTurn({
  ...
  let toolRoundLimit = toolRequirements.usesExtendedBudget
    ? MAX_TOOL_ROUNDS_WRITE
    : MAX_TOOL_ROUNDS;
  ...
  if (!requirementsIncomplete && skillsCalled.length > 0 && useTools && stripThinking(finalContent).trim()) {
  ...
  if (!wasCancelled && (roundsExhausted || requirementsIncomplete)) {
    logTeamActivity({
      ...
      missionId: ctx?.missionId || '',
      status: 'error',
      message: `Hit ${toolRoundLimit} tool round cap (${toolRequirements.usesExtendedBudget || hadWriteOp ? 'extended' : 'general'}); ${skillsCalled.length} skill${skillsCalled.length === 1 ? '' : 's'} ran; ${remainingToolSteps.length} required step${remainingToolSteps.length === 1 ? '' : 's'} remain.`,
    });
  }
  ...
  const turnStatus = wasCancelled
    ? 'cancelled'
    : (lastRoundHadToolError || roundsExhausted || requirementsIncomplete ? 'error' : 'ok');
  logTeamActivity({
    type: 'turn_done',
    agentId: currentAgentId,
    depth: Number.isFinite(ctx?.agentDepth) ? ctx.agentDepth : 0,
