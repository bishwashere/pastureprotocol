...
const CONCISE_FINAL_SYNTHESIS_INSTRUCTION =
  'Final reply rules: answer the user directly and concisely. Do not mention the completeness probe, missing-items check, retry process, internal tool rounds, or a step-by-step list of what you tried. If a tool or capability is blocked, summarize it in one short sentence only when the user must know.';
...
export async function runAgentTurn({
  ...
  // edit / home-assistant intents. The intent-planner LLM call routes these
    ...
    for (let cr = 0; cr < MAX_COMPLETENESS_RETRIES; cr++) {
      ...
      try {
        ...
        if (retryResp.toolCalls?.length) {
          ...
          const synthesis = await chatWithTools([
            ...messages,
            { role: 'user', content: CONCISE_FINAL_SYNTHESIS_INSTRUCTION },
          ], [], agentLlmOptions(`agent_turn_completeness_synthesis_r${cr}`));
