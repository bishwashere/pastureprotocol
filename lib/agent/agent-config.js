...
export function buildAgentTeamPromptBlock(callerAgentId = DEFAULT_AGENT_ID) {
  if (!isAgentVisibleInTeam(callerAgentId)) return '';
...
export function resolveEnabledSkillsForAgent(agentId, enabled) {
  ...
  if (!isAgentVisibleInTeam(agentId)) {
    return list.filter((id) => id !== 'agent-send' && id !== 'evaluate-team-capability');
  }
...
export function createAgent(agentIdInput, options = {}) {
  ...
  if (options.teamId != null) config.teamId = String(options.teamId || '').trim();
  ...
  if (config.autoLinkTeam === false || config.visibleInTeam === false || String(config.visibility || '').toLowerCase() === 'api_only') {
    config.agentMessaging = normalizeAgentMessaging({
      allow: [],
      maxDepth: 0,
      maxCallsPerTurn: 0,
...
export function getAgentMessagingPolicy(agentId = DEFAULT_AGENT_ID) {
  ...
  if (!isAgentVisibleInTeam(agentId)) return normalized;
