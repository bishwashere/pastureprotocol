...
import { getConfigPath, getWorkspaceDir, getAgentDir, getAgentConfigPath, getAgentWorkspaceDir, getGroupsDir, getGroupConfigDir, getAgentsDir, getStateDir } from './paths.js';
...
export function buildAgentTeamPromptBlock(callerAgentId = DEFAULT_AGENT_ID) {
  ...
  if (policy.allow.length) {
    ...
    lines.push('When the topic does not clearly match a specialist, evaluate-team-capability scores the team first — prefer handle-in-main over unnecessary delegation; offer a dedicated agent/Mission upgrade when no strong match.');
...
function linkNewAgentWithVisibleTeam(agentId) {
  const id = normalizeAgentId(agentId);
  if (!id || isInternalAgent(id)) return;
  const peers = listVisibleAgentIds().filter((peerId) => peerId !== id);
...
export function getAgentMessagingPolicy(agentId = DEFAULT_AGENT_ID) {
  ...
  if (normalized.allow.length === 0) {
    const peers = listVisibleAgentIds().filter((id) => id !== normalizeAgentId(agentId));
    if (peers.length > 0) {
