...
async function runBackgroundAgentTurn(task, parentCtx) {
  ...
  const ctx = {
    ...
    taskWorklogId: task.worklogId || task.id,
    sessionId,
    logKey: sessionKey,
  };
  ...
  const baseSystemPrompt = buildOneOnOneSystemPrompt(ctx.workspaceDir, { agentId }) + buildAgentTeamPromptBlock(agentId);
  ...
  return {
    ...
    taskWorklogId: turn?.taskWorklogId || task.worklogId || task.id,
...
async function executeBackgroundTask(taskId, parentCtx) {
  ...
  try {
    const { textToSend, skillsCalled, taskWorklogId, taskWorklogCheckpointCount } = await runTurnImpl(task, parentCtx);
    ...
    updateTask(taskId, {
      ...
      result: textToSend,
      skillsCalled,
      worklogId: taskWorklogId || task.worklogId || task.id,
      worklogCheckpointCount: Number(taskWorklogCheckpointCount || 0),
...
export function spawnBackgroundTask({ prompt, label, ctx }) {
  ...
  if (isDailyLimitReached({ agentId: ctx?.agentId })) {
