...
import { normalizeRequiredToolSteps } from './execution-requirements.js';
...
const VALID_TASK_FRAME_ACTIONS = new Set(['none', 'new', 'update', 'close', 'replace']);
...
const VALID_TASK_FRAME_STATUS_HINTS = new Set(['continue', 'completed', 'blocked', 'mismatch', 'waiting_user']);
const VALID_TASK_FRAME_SEED_POLICIES = new Set(['accept_candidate', 'revise_candidate', 'reject_candidate']);
const VALID_TEAM_ROUTING = new Set(['none', 'current_agent', 'delegate_to_specialist', 'coordinator_handles']);
...
export async function planUnifiedTurn({
  ...
  try {
    ...
    const requestedToolSteps = normalizeRequiredToolSteps(result.requiredToolSteps, availableSkillIds);
    const requiredSkillIds = requestedToolSteps.flatMap((step) => step.anyOfSkills);
    // Required step skills take priority so every enforced step is actually
    // exposed to the main tool loop even if the planner omitted it from skills.
    const skills = normalizeSkills(
      [...requiredSkillIds, ...(Array.isArray(result.skills) ? result.skills : [])],
      availableSkillIds,
      12,
    );
    const requiredToolSteps = normalizeRequiredToolSteps(requestedToolSteps, skills);
    const mode = VALID_MODES.has(result.mode) ? result.mode : (skills.length ? 'tool' : 'chat');
    const codeOutcomeNeedsTools = mode === 'code'
      && skills.some((id) => ['write', 'edit', 'apply-patch', 'go-write', 'exec'].includes(id));
    const teamRouting = VALID_TEAM_ROUTING.has(result.teamRouting)
      ? result.teamRouting
      : (result.needsDelegation === true ? 'delegate_to_specialist' : 'none');
    const delegationAction = VALID_DELEGATION_ACTIONS.has(result.delegationAction)
      ? result.delegationAction
      : (teamRouting === 'delegate_to_specialist' || result.needsDelegation === true ? 'delegate' : 'none');
    const requestedDelegation = teamRouting === 'delegate_to_specialist' || delegationAction === 'delegate' || result.needsDelegation === true;
    ...
    const needsWorklog = result.needsWorklog === true;

    if (needsDelegation) addSkillIfAvailable(skills, 'agent-send', availableSkillIds);
    if (needsWorklog) addSkillIfAvailable(skills, 'worklog', availableSkillIds);
    ...
    return {
      ...
      mustUseTool: result.mustUseTool === true || needsDelegation || requiredToolSteps.length > 0 || codeOutcomeNeedsTools,
      ...
      taskFrameAction,
      taskFrameSeedPolicy: VALID_TASK_FRAME_SEED_POLICIES.has(result.taskFrameSeedPolicy)
        ? result.taskFrameSeedPolicy
        : (taskFrameCandidate ? 'revise_candidate' : 'reject_candidate'),
...
export function unifiedPlanToTurnRoute(plan) {
  ...
  return {
    ...
    requiredToolSteps: Array.isArray(plan.requiredToolSteps) ? plan.requiredToolSteps : [],
    executionMode: plan.executionMode,
    usesExistingWorkIntake: plan.usesExistingWorkIntake === true,
    mustUseTool: plan.mustUseTool === true,
    needsWorklog: plan.needsWorklog === true,
