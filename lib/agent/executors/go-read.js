...
function validatePackageManagerArgs(command, argv) {
  const args = Array.isArray(argv) ? argv.map((item) => String(item)) : [];
  ...
  if (!PACKAGE_MANAGER_READ_SUBCOMMANDS.has(subcommand)) {
    return {
      ok: false,
      error: `${command} subcommand not allowed: ${firstNonFlag}. Allowed read-only subcommands: ${[...PACKAGE_MANAGER_READ_SUBCOMMANDS].sort().join(', ')}, --version.`,
    };
  }
