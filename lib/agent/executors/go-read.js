...
const ALLOWED = new Set(['ls', 'cd', 'pwd', 'cat', 'less', 'du', 'find', 'npm', 'pnpm']);
const MAX_JSON_BYTES = 5_000_000;
const MAX_JSON_SUMMARIES = 20;
const MAX_TEXT_OUTPUT_CHARS = 50_000;
const PACKAGE_MANAGER_TIMEOUT_MS = 30_000;

const PACKAGE_MANAGER_READ_SUBCOMMANDS = new Set([
  'config',
  'explain',
  'info',
  'list',
...
const PACKAGE_MANAGER_SCRIPT_SUBCOMMANDS = new Set([
  'build',
  'run',
  'run-script',
  'start',
...
export function validatePackageManagerArgs(command, argv) {
  const args = Array.isArray(argv) ? argv.map((item) => String(item)) : [];
  const firstNonFlag = args.find((item) => !item.startsWith('-'));
  const nonFlagArgs = args.filter((item) => !item.startsWith('-'));
  if (!firstNonFlag) {
    if (args.some((item) => PACKAGE_MANAGER_VERSION_FLAGS.has(item))) return { ok: true, argv: args };
    return { ok: false, error: `${command} requires a read-only subcommand or --version.` };
  }

  const subcommand = firstNonFlag.toLowerCase();
  if (PACKAGE_MANAGER_VERSION_FLAGS.has(subcommand)) {
    if (nonFlagArgs.length > 1) {
      return { ok: false, error: `${command} version is read-only here; use "${firstNonFlag}" without package/version arguments.` };
    }
  ...
  if (!PACKAGE_MANAGER_READ_SUBCOMMANDS.has(subcommand)) {
    return {
      ok: false,
      error: `${command} subcommand not allowed: ${firstNonFlag}. Allowed subcommands: ${[...PACKAGE_MANAGER_READ_SUBCOMMANDS, ...PACKAGE_MANAGER_SCRIPT_SUBCOMMANDS].sort().join(', ')}, --version.`,
    };
  }
  if (subcommand === 'config') {
    const configAction = args[args.indexOf(firstNonFlag) + 1];
    if (configAction && configAction.toLowerCase() !== 'get') {
      return { ok: false, error: `${command} config is read-only here; use "config get <key>".` };
    }
  }
  return { ok: true, argv: args };
}

function executePackageManager(ctx, args, command) {
  const validation = validatePackageManagerArgs(command, args?.argv);
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      ...
      resolve(JSON.stringify({ error: `${command} timed out after ${PACKAGE_MANAGER_TIMEOUT_MS / 1000}s.` }));
...
function executeNativeFilesystem(ctx, args) {
  ...
  try {
    if (action === 'npm' || action === 'pnpm') return executePackageManager(ctx, args, action);
