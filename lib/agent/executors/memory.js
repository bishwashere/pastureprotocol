...
import { getMemoryIndex } from '../../context/memory-index.js';
import { readChatLogDayExchanges } from '../../context/chat-log.js';
import { getAgentConfigPath, getWorkspaceDir } from '../../util/paths.js';
...
/**
 * Return one memory_search result per exchange on a calendar day (from chat-log/private).
 */
function resultsFromChatLogDay(workspaceDir, dateStr) {
  const exchanges = readChatLogDayExchanges(workspaceDir, dateStr);
  return exchanges.map((ex, i) => {
    const user = (ex.user != null ? String(ex.user) : '').trim();
    const assistant = (ex.assistant != null ? String(ex.assistant) : '').trim();
    ...
    return {
      path: ex.source || `chat-log/private/${dateStr}.jsonl`,
      startLine: i + 1,
      endLine: i + 1,
      snippet,
      score: 0.5,
...
function readAgentConfig(agentId) {
  ...
  if (!id) return {};
  try {
    const raw = readFileSync(getAgentConfigPath(id), 'utf8');
...
export function agentUsesSharedUserMemory(agentId) {
  const id = String(agentId || '').trim();
  if (!id) return false;
  const cfg = readAgentConfig(id);
  if (cfg.isolated === true) return false;
  if (cfg.sharedUserMemory === false) return false;
  return true;
}

export function resolveMemoryWorkspaceDir(ctx, fallbackWorkspaceDir) {
  const agentId = String(ctx?.agentId || '').trim();
  if (agentUsesSharedUserMemory(agentId)) return getWorkspaceDir();
  return ctx?.workspaceDir || fallbackWorkspaceDir;
}
 ...
 */
export async function executeMemory(ctx, args, toolName = 'memory_search') {
  const baseConfig = getMemoryConfig();
  const workspaceDir = resolveMemoryWorkspaceDir(ctx, baseConfig?.workspaceDir);
  const config = workspaceDir ? getMemoryConfig(null, { workspaceDir }) : baseConfig;
  if (!config) {
  ...
  const activeWorkspaceDir = workspaceDir || config.workspaceDir;
  if (!activeWorkspaceDir) return JSON.stringify({ error: 'Workspace path not set.' });
  ...
  if (toolName === 'memory_search') {
    ...
    try {
      const searchOpts = (dateFrom || dateTo || typeFilter) ? { dateFrom, dateTo, type: typeFilter } : {};
      let results = await index.search(query, searchOpts);
      ...
      const wantsFilesOrDirs = /directory|list of files|files and folders|what files|file listing|workspace contents/i.test(query);
      const hasFilesystemResults = filtered.some((r) => r.path && r.path.startsWith('filesystem/'));
      if (wantsFilesOrDirs && !hasFilesystemResults) {
        const fallbackResults = await index.search('Directory Contents list of files and folders', { ...searchOpts, maxResults: 30 });
        const fallbackFs = fallbackResults.filter((r) => r.path && r.path.startsWith('filesystem/') && r.score >= minScore).slice(0, maxResults);
        if (fallbackFs.length > 0) {
          filtered = fallbackFs;
        } else if (index.listFilesystem) {
          const listFs = index.listFilesystem(20);
          if (listFs.length > 0) filtered = listFs.slice(0, maxResults);
        }
      }
      ...
      if (asksAboutYesterday(query, args)) {
        const yesterdayResults = resultsFromChatLogDay(activeWorkspaceDir, yesterdayDateStr());
  ...
  if (toolName === 'memory_save') {
    ...
    const saveType = (args?.type && String(args.type).trim()) || null;
    ...
    const resolved = join(activeWorkspaceDir, targetRelPath);
    const rel = relative(activeWorkspaceDir, resolved);
    ...
    try {
      ...
      return JSON.stringify({ saved: true, file: targetRelPath, text, type: saveType });
    } catch (err) {
