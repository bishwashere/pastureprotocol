...
import { getConfigPath, getSecretsPath } from '../../util/paths.js';
import { normalizeSelfAlias } from '../../util/credential-utils.js';
...
async function verifyGithubMutation(method, readBack) {
  try {
  ...
  try {
    switch (action) {
      ...
      case 'create_branch': {
        ...
        const createdSha = result.object?.sha || '';
        const verification = await verifyGithubMutation('read_after_create_branch', async () => {
          const readBack = await gh(`/repos/${owner}/${name}/git/ref/heads/${encodeURIComponent(branch)}`);
          const readSha = readBack?.object?.sha || '';
          return {
            verified: Boolean(createdSha && readSha && readSha === createdSha),
            ref: readBack?.ref || `refs/heads/${branch}`,
            sha: readSha,
          };
        });
        return JSON.stringify({ created: branch, sha: createdSha, url: result.url, verification });
      }
      ...
      case 'post_comment': {
        ...
        const verification = await verifyGithubMutation('read_after_post_comment', async () => {
          const readBack = await gh(`/repos/${owner}/${name}/issues/comments/${result.id}`);
          return {
            verified: readBack?.id === result.id && String(readBack?.body || '') === body,
            id: readBack?.id,
            url: readBack?.html_url,
          };
        });
        return JSON.stringify({ id: result.id, url: result.html_url, verification });
      }
      ...
      case 'create_pr': {
        ...
        const expectedHeadRef = head.includes(':') ? head.split(':').pop() : head;
        const verification = await verifyGithubMutation('read_after_create_pr', async () => {
          const readBack = await gh(`/repos/${owner}/${name}/pulls/${result.number}`);
          return {
            verified: readBack?.number === result.number &&
              (readBack?.head?.ref === expectedHeadRef || readBack?.head?.label === head) &&
              readBack?.base?.ref === mergeBase &&
              Boolean(readBack?.html_url),
      ...
      case 'merge_pr': {
        ...
        const verification = await verifyGithubMutation('read_after_merge_pr', async () => {
          const readBack = await gh(`/repos/${owner}/${name}/pulls/${num}`);
          return {
            verified: result.merged === true &&
              (readBack?.merged === true || (result.sha && readBack?.merge_commit_sha === result.sha)),
            number: readBack?.number,
            state: readBack?.state,
            merge_commit_sha: readBack?.merge_commit_sha || '',
          };
        });
        return JSON.stringify({ merged: result.merged, sha: result.sha, message: result.message, verification });
      }
