...
const MAX_NODE_SCRIPT_CHARS = 200_000;
...
function timeoutFor(args, config) {
  const requestedTimeout = Number(args?.timeoutMs);
  return Number.isFinite(requestedTimeout)
    ? Math.max(1_000, Math.min(MAX_TIMEOUT_MS, requestedTimeout))
    : config.timeoutMs;
}

function installRoot() {
  const configured = String(process.env.PASTURE_INSTALL_DIR || '').trim();
  if (configured) return resolve(expandTilde(configured));
  return resolve(dirname(fileURLToPath(import.meta.url)), '..', '..', '..');
}

function envFromFile(envFile, cwd) {
  const requested = String(envFile || '').trim();
  if (!requested) return { ok: true, env: {} };
  const path = resolve(cwd, expandTilde(requested));
  try {
    return { ok: true, env: normalizeEnv(dotenv.parse(readFileSync(path))) };
  } catch (err) {
    return { ok: false, error: `Could not load envFile ${path}: ${err?.message || err}` };
  }
}

async function executeNodeScript(ctx, args, config) {
  const validation = validateCommand('node', config.mode, config.allowlist);
  if (!validation.ok) return JSON.stringify({ error: validation.error });
  const source = typeof args?.source === 'string' ? args.source : '';
  if (!source.trim()) return JSON.stringify({ error: 'source is required for node_script.' });
  if (source.length > MAX_NODE_SCRIPT_CHARS) {
    return JSON.stringify({ error: `node_script source exceeds ${MAX_NODE_SCRIPT_CHARS} characters.` });
  }

  const cwd = args?.cwd
    ? resolve(expandTilde(String(args.cwd)))
    : (ctx?.workspaceDir || process.cwd());
  const loadedEnv = envFromFile(args?.envFile, cwd);
  if (!loadedEnv.ok) return JSON.stringify({ error: loadedEnv.error });

  let tempDir = '';
  try {
    // Locating the transient module under the Pasture install lets ESM resolve
    // Pasture-owned dependencies while the child process still runs in the
    // target project's cwd. The directory is always removed in finally.
    tempDir = mkdtempSync(join(installRoot(), '.pasture-node-script-'));
    const scriptPath = join(tempDir, 'script.mjs');
    writeFileSync(scriptPath, source, { encoding: 'utf8', mode: 0o600 });
    const argv = [
      scriptPath,
      ...(Array.isArray(args?.argv) ? args.argv.map((item) => String(item)) : []),
    ];
    const env = buildEnv(config.pathPrepend, {
      ...loadedEnv.env,
    ...
    const result = await spawnWithTimeout(validation.command, argv, {
      cwd,
      env,
      timeoutMs: timeoutFor(args, config),
 ...
 */
export async function executeExec(ctx, args = {}, toolName = '') {
  const config = getExecConfig();
  const action = toolName === 'exec_node_script'
    ? 'node_script'
    : String(args?.action || '').trim();
  if (action === 'node_script') {
    return executeNodeScript(ctx, args, config);
  }
  ...
  const cwd = args.cwd
    ? resolve(expandTilde(String(args.cwd)))
    : (ctx?.workspaceDir || process.cwd());
  const timeoutMs = timeoutFor(args, config);
