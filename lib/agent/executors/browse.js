...
/**
 * Session key. Two agents talking in the same group chat MUST NOT share one
 * Playwright tab — one agent's login or scroll position would silently affect
 * the other (audit finding #13). Key by jid + agentId so each agent gets its
 * own browser session.
 *
 * @param {object} ctx
 * @returns {string}
...
function sessionKey(ctx) {
  ...
  const agentId = ctx?.agentId != null ? String(ctx.agentId) : '';
  if (jid && agentId) return `${jid}|${agentId}`;
  if (jid) return jid;
