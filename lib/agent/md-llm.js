 ...
 * @param {number} [opts.maxTokens] - Optional token cap override for this prompt.
 * @param {boolean} [opts.skipLocal] - Optional local-model skip for prompts that need cloud reliability.
...
export async function runMdPrompt({
  ...
  try {
    raw = await chatFn(
      ...
      { agentId, purpose: purpose || `md_prompt:${promptName}`, maxTokens, skipLocal },
    );
  } catch (err) {
