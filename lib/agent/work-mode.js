...
export async function classifyWorkModeToggle({ userText, currentMode, agentId, llmChat = null } = {}) {
  ...
  let toggle = VALID_TOGGLES.has(result.toggle) ? result.toggle : 'no_change';
  if (mode === 'multi' && toggle === 'enable') toggle = 'no_change';
  if (mode === 'single' && toggle === 'disable') toggle = 'no_change';
