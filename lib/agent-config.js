 ...
 */
export function resolveAgentReference(input) {
...
export function normalizeAgentAllowList(allow) {
  ...
  for (const entry of allow) {
    ...
    if (resolved && known.has(resolved) && !out.includes(resolved)) out.push(resolved);
  }
  return out;
}
...
export function buildAgentTeamPromptBlock(callerAgentId = DEFAULT_AGENT_ID) {
  const ids = listAgentIds().filter((id) => id !== 'reflector');
  if (ids.length <= 1) return '';
  const policy = getAgentMessagingPolicy(callerAgentId);
  const lines = [
    '# Agent team',
    'Always use the canonical agent id for agent-send (not display names alone).',
  ];
  for (const id of ids) {
