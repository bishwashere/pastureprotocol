...
export function buildAgentTeamPromptBlock(callerAgentId = DEFAULT_AGENT_ID) {
  ...
  if (policy.allow.length) {
    ...
    lines.push('When the topic does not clearly match a specialist, evaluate-team-capability scores the team first — prefer handle-in-main over unnecessary delegation; offer a dedicated agent/Goal upgrade when no strong match.');
