 ...
 * Set TELEGRAM_BOT_TOKEN in env (or in ~/.pasture/.env) to enable.
 */

import TelegramBot from 'node-telegram-bot-api';
import { getErrorMessageForLog } from '../util/user-error.js';
...
export function initBot(token) {
  ...
  bot.on('polling_error', (err) => {
    ...
    if (msg.includes('409') || msg.includes('Conflict') || msg.includes('getUpdates')) {
      console.log('[Telegram] Another Pasture Protocol is already using this bot; this process won\'t get Telegram messages. To use this process instead, stop the other: pasture stop');
...
/**
 * Telegram chat actions are temporary, so keep refreshing "typing" until stopped.
 * @param {import('node-telegram-bot-api')} bot
 * @param {number|string} chatId
 * @returns {() => void} Stop function
 */
export function startTypingIndicator(bot, chatId) {
  if (!bot || typeof bot.sendChatAction !== 'function' || chatId == null) {
    return () => {};
  }
  ...
  const sendTyping = () => {
    ...
    bot.sendChatAction(chatId, 'typing').catch((err) => {
      console.log('[Telegram] sendChatAction failed:', getErrorMessageForLog(err).slice(0, 80));
    });
  };

  sendTyping();
  const timer = setInterval(sendTyping, TYPING_ACTION_INTERVAL_MS);
