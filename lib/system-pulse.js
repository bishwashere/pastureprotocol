...
import { existsSync, readFileSync, writeFileSync, mkdirSync, copyFileSync, statSync, readdirSync, appendFileSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { getStateDir, getWorkspaceDir, getConfigPath, getCronStorePath } from './paths.js';
import { chat as llmChat, isDailyLimitReached } from '../llm.js';
import { stripThinking } from './agent.js';
import { readChatLogsForLocalDates } from './chat-log.js';
import { getResolvedTimezone } from './timezone.js';
import { startRequestTrace, runWithRequestTrace, logRequestStart, logRequestEnd, traceAsyncStep } from './request-timing.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const PROMPT_DIR = join(__dirname, 'templates');
...
function getSelfEditsLogPath() {
  return join(getStateDir(), 'self-edits.log');
}
 ...
 * Timestamps in daemon.log are UTC ISO strings like [2026-06-14T12:31:28].
 */
function hasRecentLlmActivity(withinMs = 20 * 60_000) {
  const logPath = join(getStateDir(), 'daemon.log');
...
export async function runHealthCheck() {
  const logPath = join(getStateDir(), 'daemon.log');
  const errPath = join(getStateDir(), 'daemon.err');
...
export async function runOutputPatternCheck() {
  ...
  const prompt = [
    'You are a system self-improvement agent reviewing recent conversation outputs.',
    'Your goal: find at most ' + cfg.maxPatternsPerRun + ' high-impact behavioral patterns that should be fixed.',
    '',
    'Current SOUL.md:',
    currentSoul.slice(0, 2000),
    '',
    'Recent exchanges (prioritized by signal — tool use, long replies, corrections):',
    exchangeBlock,
    '',
    'For each pattern found, propose a specific edit to SOUL.md or a skill SKILL.md file.',
    'Return JSON only:',
    '{',
    '  "patterns": [',
    '    {',
    ...
    '',
    'Rules:',
    '- Only propose changes that fix clear, repeated patterns.',
    '- Do not add rules the base model already knows.',
    '- Keep edits minimal and specific.',
    '- If no actionable patterns, return {"patterns": []}.',
  ].join('\n');
  ...
  try {
    ...
    const parsed = JSON.parse(stripThinking(raw || '').trim().replace(/^```[a-z]*\n?/i, '').replace(/\n?```$/i, '').trim());
    patterns = Array.isArray(parsed.patterns) ? parsed.patterns.slice(0, cfg.maxPatternsPerRun) : [];
  } catch (err) {
    console.log('[system-pulse] Pattern check LLM failed:', err.message);
...
async function selfCritiqueEdit(pattern, currentSoul, threshold) {
  try {
    const prompt = [
      'You are a safety reviewer. A system wants to self-edit its own instructions.',
      '',
      'Proposed edit:',
      `File: ${pattern.file}`,
      `Action: ${pattern.action}`,
      `Rationale: ${pattern.rationale}`,
      pattern.oldText ? `Replace: "${pattern.oldText.slice(0, 200)}"` : '(append)',
      `New text: "${(pattern.newText || '').slice(0, 300)}"`,
      '',
      'Current SOUL.md (first 1000 chars):',
      (currentSoul || '').slice(0, 1000),
      '',
      'Is this change an actual improvement? Is it safe? Does it conflict with existing instructions?',
      'Return JSON only: {"approved": boolean, "confidence": 0.0-1.0, "reason": "one sentence"}',
    ].join('\n');
    ...
    return { approved: false, confidence: 0, reason: `critique failed: ${err.message}` };
  }
}
