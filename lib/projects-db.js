 ...
 *
 * DB file: $COWCODE_STATE_DIR/projects.db  (default: ~/.cowcode/projects.db)
 *
 * Auth: username/password for the Projects page.
 *   Config: config.json -> dashboard.projects.username / .password
 *   Env:    COWCODE_PROJECTS_USERNAME / COWCODE_PROJECTS_PASSWORD
 *   Default: admin / cowcode
 */
...
export function getProject(id) {
  return getProjectsDb().prepare('SELECT * FROM projects WHERE id = ?').get(id);
}

export function createProject({ name, description = '', url = '' }) {
  const db = getProjectsDb();
  const now = Date.now();
  const result = db.prepare(
    'INSERT INTO projects (name, url, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?)'
  ).run(name, normalizeProjectUrl(url), description, now, now);
  return getProject(result.lastInsertRowid);
}

export function updateProject(id, { name, description, url }) {
  const db = getProjectsDb();
  const now = Date.now();
  const existing = getProject(id);
  if (!existing) return null;
  const nextUrl = url !== undefined ? normalizeProjectUrl(url) : (existing.url || '');
  db.prepare('UPDATE projects SET name=?, url=?, description=?, updated_at=? WHERE id=?')
    .run(name, nextUrl, description, now, id);
...
// ── Auth ──────────────────────────────────────────────────────────────────────
// Simple in-memory session tokens. Sessions expire after 24 h.

const SESSIONS = new Map(); // token → expiresAt
const SESSION_TTL_MS = 24 * 60 * 60 * 1000;

export function getProjectsCredentials(config = {}) {
  const cfg = (config.dashboard && config.dashboard.projects) || {};
  const username = process.env.COWCODE_PROJECTS_USERNAME || cfg.username || 'admin';
  const password = process.env.COWCODE_PROJECTS_PASSWORD || cfg.password || 'cowcode';
  return { username, password };
}

export function projectsLogin(username, password, config) {
...
export function validateProjectsToken(token) {
