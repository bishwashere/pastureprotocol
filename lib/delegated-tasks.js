...
export function updateDelegatedSubgoalProgress(opts = {}) {
  ...
  if (opts.note) {
    ...
    const existing = flattenSubgoals(goal.subgoals || []).find((s) => s.id === subgoalId);
  ...
  const activity = `Delegated subgoal ${subgoalId} ${activityByStatus[status] || 'updated'}`;
...
export function recordDelegatedSubgoalReply(delegatedTask, opts = {}) {
