...
import { getMission, listMissions, updateMission } from './missions.js';
import { resolveMissionForUserTurn } from './missions-context.js';
import { isProjectMetaInquiry, looksLikeConcreteWorkRequest } from './task-intent.js';
...
function patchTaskInTree(tasks, taskId, patch) {
  let found = false;
  const next = (tasks || []).map((sg) => {
    if (String(sg.id || '') === taskId) {
      found = true;
      return { ...sg, ...patch };
    }
    if (Array.isArray(sg.tasks) && sg.tasks.length) {
      const nested = patchTaskInTree(sg.tasks, taskId, patch);
      if (nested.found) {
        found = true;
...
export function shouldPersistDelegatedTask(opts = {}) {
  ...
  if (isProjectMetaInquiry(message) || isProjectMetaInquiry(expectedOutput)) return false;
  if (isProjectMetaInquiry(combined) && !looksLikeConcreteWorkRequest(message)) return false;
  if (expectedOutput && !isProjectMetaInquiry(expectedOutput)) return true;
  if (title && looksLikeConcreteWorkRequest(title)) return true;
  return looksLikeConcreteWorkRequest(message);
}
...
function classifyReplyStatus(reply, expectedOutput = '') {
  const text = String(reply || '').trim();
  if (!text) return { status: 'blocked', progress: 0, reason: 'empty reply' };

  if (/\b(blocked|cannot continue|can't continue|unable to continue|failed|error|timeout)\b/i.test(text)) {
    return { status: 'blocked', progress: 25, reason: 'specialist reported a blocker' };
  }
  if (/\b(waiting for|depends on|dependency|need access|missing access|need credentials|needs credentials|api key)\b/i.test(text)) {
