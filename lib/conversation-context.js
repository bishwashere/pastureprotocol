...
import { isWorkOrDiscoveryRequest } from './missions-context.js';
...
export function buildAnswerCompletenessProbePrompt(userText, assistantAnswer, historyMessages) {
  ...
  const workHint = isWorkOrDiscoveryRequest(userText)
    ? '- The user asked to learn or continue **work** (find out, what is it about, etc.). Mark **incomplete** if the assistant only asked them to pick a source (GitHub vs path vs tracker) without using tools first when an **Active mission** or URL was available.\n'
    : '';
  return (
    `${historySection}` +
