 ...
 * Keyword scoring first; LLM semantic router when keyword match is weak.
 */
export async function buildDelegationContext(opts = {}) {
  ...
  const keywordRec = ctx.teamCapability.recommendation || {};
  if (!needsLlmDelegationRouter(ctx.teamCapability, keywordRec)) {
    return ctx;
  }
