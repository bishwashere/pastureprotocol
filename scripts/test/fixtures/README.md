...
Tests use `prepareStateFromFixture()` from `scripts/test/support/test-fixture-state.js`: it copies your `~/.pasture` config and `.env` into a temp dir and overlays this fixture (workspace + cron), so the app has both fixture data and your LLM/config.
