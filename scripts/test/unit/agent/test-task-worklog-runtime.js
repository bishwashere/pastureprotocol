...
assert(/const normalizedRequiredSteps = worklogRequired\s*\?\s*\[\]/.test(agent),
  'long checkpointed runs are not blocked by exact planner contracts');
assert(/if \(!worklogRequired && !requirementsIncomplete/.test(agent),
  'long checkpointed runs skip the semantic completeness reviewer');
assert(/MAX_LONG_TASK_RUNTIME_MS/.test(agent) && /LONG_TASK_TIMEOUT/.test(agent),
  'long checkpointed runs stop on one runtime window and return a partial report');
...
assert(/skillId === 'worklog' && action === 'checkpoint'[\s\S]{0,700}?MAX_TOOL_ROUNDS_WORKLOG/.test(agent),
